...
function parseCardHtml(id: number, html: string, baseOfficial: OfficialCard): Partial<MapCard> {
  ...
  // reminder" blocks (e.g. Ultra Ball's "[物品規則]") use the same bracket convention but
  // aren't real card data — skip those specifically, and log (not silently drop) anything else
  // bracket-prefixed so a genuinely new category doesn't get misfiled unnoticed.
