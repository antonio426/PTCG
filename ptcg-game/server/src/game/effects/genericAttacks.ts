...
export interface GenericAttackOutcome {
  ...
  deckSearchSupertypeToHand?: 'Pokémon' | 'Item' | 'Supporter' | 'Stadium';
  ...
  /** Reveal the bottom `count` cards of the attacker's own deck; damage = (how many of them are
   * Pokémon printing `attackName`) x amount. Those Pokémon go back into the deck (shuffled), the
   * rest are discarded. */
  revealBottomAttackHolderScaledDamage?: { count: number; attackName: string; amount: number };
  /** Pay Energy off the attacker for a rider: `amount` extra damage and/or a Special Condition on
   * the defender. `type` narrows which Energy pays, `all` empties the attacker, `toDeck` returns
   * them (shuffled) instead of discarding. 「若希望」 texts resolve as "do it" like everywhere else. */
  selfEnergyPaymentRider?: { count?: number; type?: string; all?: boolean; toDeck?: boolean; amount?: number; status?: StatusCondition };
  /** One coin per opponent Pokémon; every heads takes `amount` (no weakness/resistance). */
  coinPerOpponentPokemonDamage?: { amount: number };
  /** Move every damage counter off the opponent's Bench onto their Active. */
  moveOpponentBenchCountersToActive?: boolean;
  /** Discard every Energy on the attacker, then place `counters` on ONE opponent Pokémon. */
  discardAllSelfEnergyForCounters?: { counters: number };
  /** Return `count` of the attacker's own 【type】 Energy to hand. */
  returnSelfTypedEnergyToHand?: { type: string; count: number };
  ...
  /** KO the defender outright (a real Knock Out — prizes are taken). */
  koDefender?: boolean;
  /** DISCARD the defender with everything attached — not a KO, so NO prizes are taken. */
  discardDefenderEntirely?: boolean;
  /** DISCARD the attacker itself with everything attached — not a KO, no prizes for the opponent. */
  discardSelfEntirely?: boolean;
  /** Return the attacker plus attachments/stack to its owner's DECK (reshuffled). */
  ...
  /** Heal `amount` off 1 random own-Bench Pokémon whose name OR paradox tag matches `name`. */
  healBenchNamedAmount?: { name: string; amount: number };
  /** Keep `keep` random opponent Bench Pokémon; every OTHER benched one (with attachments and
   * stack) is shuffled into their deck. */
  shuffleOpponentBenchExceptCount?: { keep: number };
  /** Show every discard-pile card whose name includes `name`; place (that count × per) damage
   * counters on 1 random opponent Pokémon, then shuffle those shown cards into the deck. */
  discardNamedToDeckCountersOnOpponent?: { name: string; per: number };
  /** Discard `count` attached Energy whose card name includes `name` from the attacker; if at
   * least one was discarded, also apply `thenDiscardDefender`. */
  discardSelfNamedEnergy?: { name: string; count: number; thenDiscardDefender?: boolean };
  /** Take up to `count` cards whose name includes `name` from the discard pile to hand. */
  discardPileSearchNamedToHandCount?: { name: string; count: number };
  /** De-evolve `count` random opponent evolved Pokémon by one Stage; the removed card returns
   * to the opponent's hand. */
  devolveOpponentToHandCount?: number;
  /** Flat no-w/r damage to every damaged Pokémon on BOTH sides except the attacker itself. */
  damageAllDamagedBothSidesExceptSelf?: { amount: number };
  /** Timed effects processed by the between-turns sweep rather than the attack pipeline. */
  opponentDelayedEffect?: { kind: 'delayedKo' | 'delayedCounters' | 'delayedDiscard'; amount?: number; turnOffset: number };
  /** PRE-DAMAGE (resolved in applyAttackOutcome's early scaled-damage section, because the
   * numbers depend on hidden zones the text-side resolver can't see): */
  /** Mill the top `count` of BOTH decks; +`per` damage per Energy card among them. */
  millBothTopScaledDamage?: { count: number; per: number };
  /** Attach every hand card whose name includes `handName` to the attacker, then damage =
   * (attached Energy of `type` on the attacker) × per. */
  attachAllNamedFromHandThenTypedScaled?: { handName: string; type: string; per: number };
  /** Place up to `max` counters on the attacker itself (stopping short of self-KO); damage =
   * placed × per. */
  selfCountersScaledDamage?: { max: number; per: number };
  /** +`per` damage for each Energy of `type` attached across own Pokémon whose name includes `family`. */
  familyTypedEnergyScaledBonus?: { family: string; type: string; per: number };
  /** The attacker switches with a random own-Bench Pokémon of this flat type. */
  selfSwitchToTypedBench?: { type: string };
  /** Discard every Item and Pokémon Tool card from the opponent's hand. */
  discardOpponentHandItemsAndTools?: boolean;
  /** Flat no-w/r damage to `count` random opponent BENCH 「寶可夢【ex】」. */
  opponentExBenchFlatDamage?: { count: number; amount: number };
  /** Attach every Basic Energy card in the attacker's own hand to random own Pokémon. */
  attachAllBasicEnergyFromHand?: boolean;
  /** Attach up to `count` hand cards whose name includes `name` (Energy) to random own Pokémon. */
  attachNamedFromHandCount?: { name: string; count: number };
  /** Coin flip: heads KOs the defender if it is a Basic; tails KOs a random benched opponent Basic. */
  koOpponentBasicCoinSplit?: boolean;
  /** If the attacker's side has exactly 1 prize card left, they win the game outright. */
  winGameIfOnePrizeLeft?: boolean;
  /** KO a random opponent Pokémon carrying at least this many damage counters. */
  koOpponentWithCountersAtLeast?: { counters: number };
  /** Resolve a random one of the defender's own printed attacks as this attack. */
  copyDefenderRandomAttack?: boolean;
  /** Flat no-w/r damage to every own in-play Pokémon whose name is in `names`. */
  damageOwnFamilyAll?: { names: string[]; amount: number };
  /** Both players draw this many cards. */
  bothDrawCount?: number;
  /** Give every own 「未來」 Pokémon damage immunity vs ex attackers next opponent turn. */
  timedImmunityAllOwnFuture?: boolean;
  /** Timed 'cantAttack' applied to a whole side ('own' or 'both'), optionally only to Pokémon
   * holding at most `maxEnergy` Energy (evaluated at apply time — a later attach doesn't lift it,
   * the documented simplification). */
  timedCantAttackAll?: { side: 'own' | 'both'; maxEnergy?: number; turnOffset: number };
  /** Lets statusToInflict land even on a 0-damage use (for "all tails → Confuse" style texts
   * whose status is the miss consolation, not a rider on the hit). */
  statusEvenAtZeroDamage?: boolean;
  /** Shuffle N random own-Bench Pokémon (with attachments and stack) into the deck. */
  shuffleOwnBenchToDeckCount?: number;
  /** Attach up to N Energy cards (any) from the hand to the attacker itself. */
  attachAnyEnergyFromHandToSelfCount?: number;
  /** Place `counters` damage counters on every ability-holding Pokémon on BOTH sides. */
  countersOnAllAbilityHolders?: { counters: number };
  /** Search the deck for up to N Basic Energy, attach to the attacker itself, reshuffle. */
  deckSearchBasicEnergyToSelfCount?: number;
  /** Take up to N Basic Pokémon from the OPPONENT's hand and bench them on their side. */
  benchOpponentHandBasicsCount?: number;
  /** Take up to N Trainer cards from the own discard pile to hand. */
  discardPileSearchTrainerToHandCount?: number;
  /** Place counters on every opponent BENCH Pokémon down to exactly N remaining HP each. */
  setAllOpponentBenchRemainingHp?: number;
  /** Both Actives are Knocked Out (each side takes its prizes). */
  koBothActives?: boolean;
  /** Discard the attacker's own attached Tool (first slot) — the matcher verified one exists. */
  discardSelfTool?: boolean;
  /** Place N damage counters on 1 random opponent BENCH Pokémon. */
  placeCountersOnRandomOpponentBench?: number;
  /** Attach up to N Energy from the OPPONENT's discard pile onto their own Pokémon. */
  attachOpponentDiscardEnergyToTheirPokemonCount?: number;
  /** Attach 1 Basic Energy of this type from the own DISCARD PILE to each own Benched Pokémon. */
  discardPileTypedEnergyToAllBenchEach?: string;
  /** Discard up to N Pokémon Tool cards attached across the opponent's field. */
  discardOpponentFieldToolsCount?: number;
  /** Shuffle up to `count` discard-pile cards whose name includes `name` into the deck. */
  discardPileNamedToDeckCount?: { name: string; count: number };
  /** PRE-DAMAGE: mill own top `count`; damage += (Energy cards among them) × per. */
  selfMillEnergyScaledDamage?: { count: number; per: number };
  /** Search the deck for up to N Energy cards of ANY kind, add to hand, reshuffle. */
  deckSearchAnyEnergyToHandCount?: number;
  /** Take this many of the OWN prize cards into hand. */
  takeOwnPrizeCount?: number;
  /** Search the deck for N Energy cards of any kind and attach to the attacker, reshuffle. */
  deckSearchAnyEnergyToSelfCount?: number;
  /** Both players shuffle hand into deck, then each draws N. */
  bothShuffleHandDrawCount?: number;
  /** Reveal the OPPONENT's top N deck cards; Basics among them fill their Bench, rest reshuffled. */
  revealOpponentTopBenchBasicsCount?: number;
  /** Gust: pull 1 random opponent Benched Pokémon into their Active, then splash it. */
  gustOpponentBenchThenSplash?: { splash: number };
  /** Search the deck for up to N Pokémon of ANY stage, place onto the Bench, reshuffle. */
  deckSearchPokemonToBenchCount?: number;
  /** Take up to N Basic Pokémon from the own discard pile onto the Bench. */
  discardPileSearchBasicToBenchCount?: number;
  /** Flat no-w/r damage to 1 random opponent Pokémon holding a Special Energy. */
  opponentSpecialEnergyHolderSplash?: { amount: number };
  /** Both players may attach up to N Basic Energy from their hands to their own Pokémon. */
  bothAttachHandBasicsCount?: number;
  /** Heal every Pokémon on BOTH sides by this amount. */
  healAllBothSidesAmount?: number;
  /** KO every opponent Pokémon whose REMAINING HP is at most this. */
  koAllOpponentRemainingHpAtMost?: number;
  /** Blind-discard the opponent's hand down to this many cards. */
  discardOpponentHandDownTo?: number;
  /** Discard `count` attached Energy of exactly this flat type from the DEFENDER. */
  discardOpponentTypedEnergy?: { type: string; count: number };
  /** Double the damage counters on every opponent Pokémon. */
  doubleAllOpponentCounters?: boolean;
  /** KO 1 random opponent Pokémon (active or bench). */
  koRandomOpponent?: boolean;
  /** Fully heal 1 own Pokémon (the most damaged; bench-only when set). */
  healOneOwnFullHeal?: { benchOnly: boolean };
  /** KO the lowest-remaining-HP Pokémon on either side, the attacker itself excluded. */
  koLowestRemainingHpExceptSelf?: boolean;
  /** Heal every own BASIC Pokémon by this amount. */
  healAllOwnBasicsAmount?: number;
  /** Return N own BENCH Pokémon (with attachments and stack) to hand — 平穩境地-gated. */
  returnOwnBenchToHandCount?: number;
  /** Evolve 1 own in-play Pokémon using a matching evolution from the deck, reshuffle. */
  evolveOneFieldFromDeck?: boolean;
  /** Take up to N cards of ANY kind from the own discard pile to hand. */
  discardPileSearchAnyToHandCount?: number;
  /** Draw N cards from the BOTTOM of the own deck. */
  drawFromBottomCount?: number;
  /** Reveal the top N deck cards; every Pokémon among them goes to hand, rest reshuffled. */
  revealTopPokemonToHandCount?: number;
  /** Quadruple the damage counters on `count` random damaged opponent Pokémon. */
  quadrupleCountersOnOpponents?: { count: number };
  /** Reveal the opponent's top `count` deck cards; resolve a random attack printed on a Pokémon
   * among them as this attack, then shuffle the revealed cards back. */
  copyFromOpponentDeckTop?: { count: number };
  /** Add N to this side's bonusPrizeNextKo (「多獲得N張獎賞卡」 next-KO riders, approximated to
   * the existing any-next-KO counter). */
  setBonusPrizeNextKo?: number;
  /** Additional timed effects for the DEFENDER when one slot isn't enough. */
  opponentTimedEffects?: TimedEffectDescriptor[];
  /** Discard the opponent's own Stadium if one is in play; if one was discarded, they can't play
   * Stadium cards next turn. */
  discardOpponentStadiumThenLock?: boolean;
  /** Lock the OPPONENT out of playing Supporters / evolving next turn. */
  supporterLockOpponentNextTurn?: boolean;
  evolutionLockOpponentNextTurn?: boolean;
  /** Bench 1 Basic from the deck, then move 1 of the attacker's Energy onto the newcomer. */
  benchBasicFromDeckThenMoveEnergy?: boolean;
}
...
export interface AttackBoardContext {
  ...
  ownHandCount: number;
  /** The attacker's own current Special Conditions. */
  attackerStatusConditions: string[];
  /** Whether the defender has a Pokémon Tool attached (either slot). */
  defenderHasTool: boolean;
  /** Whether the defender has a Special Energy card attached. */
  defenderHasSpecialEnergy: boolean;
  /** Whether the attacker itself has a Pokémon Tool attached. */
  attackerHasTool: boolean;
  /** Special Energy cards attached to the attacker itself. */
  attackerSpecialEnergyCount: number;
  /** Special Energy cards attached across the opponent's whole field. */
  opponentFieldSpecialEnergyCount: number;
  /** Attack names THIS attacker declared during its owner's previous turn. */
  attackerAttacksUsedLastTurn: string[];
  /** True when an OWN Ancient Pokémon other than the attacker attacked last own-turn. */
  ownOtherAncientAttackedLastTurn: boolean;
  /** Supporter card names this side played this turn (moves.playTrainer records them). */
  ownSupporterNamesPlayedThisTurn: string[];
  /** Cards in the own discard pile carrying the Ancient tag. */
  ownDiscardAncientCount: number;
  /** Distinct Pokémon types across the attacker's own hand. */
  ownHandPokemonTypeCount: number;
  /** The attacker's own remaining HP (effective max minus damage). */
  ...
  defenderResistanceTypes: string[];
  /** Names of own in-play Pokémon (active+bench) that carry at least 1 damage counter. */
  ownFieldDamagedNames: string[];
  /** Damage counters summed per flat type across the own BENCH. */
  ...
  /** Prizes the OPPONENT took during their last turn. */
  opponentPrizesTakenLastTurn: number;
...
export const NEUTRAL_BOARD: AttackBoardContext = {
  ...
  attackerPromotedFromBenchThisTurn: false, attackerHealedThisTurn: false, opponentPrizesTakenLastTurn: 0, attackerDamageTakenLastTurn: 0, ownDiscardEnergyCounts: {},
...
export function resolveGenericAttackEffect(text: string, damageField: string, board: AttackBoardContext): GenericAttackOutcome | undefined {
  ...
  /* ---- Round 4: the handwritten long tail (one template per remaining printed text family). ---- */

  // 將對手的戰鬥寶可夢【昏厥】。(also the payload of hand-cost KO attacks below)
  if (/^將對手的戰鬥寶可夢【昏厥】。$/.test(t)) return { baseDamage: parseBaseNumber(damageField), koDefender: true };

  // 從自己的手牌將N張「X」卡丟棄，(payload)。若無法丟棄(N張)?，則這個招式失敗。
  ...
  if (/^將這隻寶可夢身上附加的能量卡全部丟棄。在下個對手的回合結束時，受到這個招式的寶可夢會【昏厥】。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), discardAllSelfEnergy: true, opponentDelayedEffect: { kind: 'delayedKo', turnOffset: 1 } };
  }

  // 在下個對手的回合結束時，在受到這個招式的寶可夢身上放置N個傷害指示物。
  m = t.match(/^在下個對手的回合結束時，在受到這個招式的寶可夢身上放置(\d+)個傷害指示物。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), opponentDelayedEffect: { kind: 'delayedCounters', amount: parseInt(m[1], 10), turnOffset: 1 } };

  // 在下個對手的回合結束時，將受到這個招式的寶可夢與附加的卡全部丟棄。
  if (/^在下個對手的回合結束時，將受到這個招式的寶可夢與附加的卡全部丟棄。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), opponentDelayedEffect: { kind: 'delayedDiscard', turnOffset: 1 } };
  }

  // 若希望，將這隻寶可夢身上附加的能量卡全部放回手牌，增加N點傷害。(auto-taken when any Energy
  // is attached — the bonus lands in baseDamage here, the physical return at the apply site)
  m = t.match(/^若希望，將這隻寶可夢身上附加的能量卡全部放回手牌，增加(\d+)點傷害。$/);
  if (m) {
    if (board.attackerTotalEnergyCount === 0) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField) + parseInt(m[1], 10), returnAllSelfEnergyToHandBonus: { amount: parseInt(m[1], 10) } };
  }

  // 將自己的備戰區的N隻「X」寶可夢恢復「M」HP。
  m = t.match(/^將自己的備戰區的1隻「(.+?)」寶可夢恢復「(\d+)」HP。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), healBenchNamedAmount: { name: m[1], amount: parseInt(m[2], 10) } };

  // 選擇N隻對手的備戰寶可夢。然後，將對手的沒有選擇的所有備戰寶可夢與附加的卡，全部放回對手的牌庫並重洗。
  m = t.match(/^選擇(\d+)隻對手的備戰寶可夢。然後，將對手的沒有選擇的所有備戰寶可夢與附加的卡，全部放回對手的牌庫並重洗。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), shuffleOpponentBenchExceptCount: { keep: parseInt(m[1], 10) } };

  // 選擇N隻對手的備戰寶可夢，將那隻寶可夢與附加的卡全部放回對手的牌庫並重洗。
  m = t.match(/^選擇(\d+)隻對手的備戰寶可夢，將那(?:隻|些)寶可夢與附加的卡，?全部放回對手的牌庫並重洗。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), shuffleOpponentBenchToDeckCount: parseInt(m[1], 10) };

  // 這個招式只可在後攻玩家的最初回合使用。(rest) — timing enforced in canAttack; resolve the rest.
  m = t.match(/^這個招式只可在後攻玩家的最初回合使用。(.+)$/);
  if (m) {
    const rest = resolveGenericAttackEffect(m[1], damageField, board);
    if (rest) return rest;
  }

  // 將這隻寶可夢身上附加的能量卡，全部改附於N隻備戰寶可夢身上。
  if (/^將這隻寶可夢身上附加的能量卡，全部改附於\d+隻備戰寶可夢身上。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), moveSelfEnergyToRandomBenchCount: 99 };
  }

  // 將對手的戰鬥寶可夢身上附加的「寶可夢道具」卡與「特殊能量」卡全部丟棄。
  if (/^將對手的戰鬥寶可夢身上附加的「寶可夢道具」卡與「特殊能量」卡全部丟棄。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), discardOpponentTool: true, discardOpponentSpecialEnergyCount: 99 };
  }

  // 在給對手看過自己的棄牌區的所有「X」卡後，將與其張數×N個的相同數量的傷害指示物，放置於對手的1隻寶可夢身上。然後，將給對手看過的能量卡放回牌庫並重洗。
  m = t.match(/^在給對手看過自己的棄牌區的所有「(.+?)」卡後，將與其張數×(\d+)個的相同數量的傷害指示物，放置於對手的1隻寶可夢身上。然後，將給對手看過的能量卡放回牌庫並重洗。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardNamedToDeckCountersOnOpponent: { name: m[1], per: parseInt(m[2], 10) } };

  // 在下個對手的回合，這隻寶可夢不會受到「古代/未來」寶可夢招式的傷害。
  m = t.match(/^在下個對手的回合，這隻寶可夢不會受到「(古代|未來)」寶可夢招式的傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), selfTimedEffect: { kind: 'damageImmune', vsSubtype: m[1] === '古代' ? 'Ancient' : 'Future', turnOffset: 1 } };

  // 將這隻寶可夢與附加的卡，全部放回自己的牌庫(並重洗)。(optionally followed by a hand search)
  m = t.match(/^將這隻寶可夢與附加的卡，全部放回自己的牌庫(?:並重洗)?。(?:然後，從牌庫任意選擇最多(\d+)張卡加入手牌。並且重洗牌庫。)?$/);
  if (m) {
    const outcome: GenericAttackOutcome = { baseDamage: parseBaseNumber(damageField), returnSelfAndAttachmentsToDeck: true };
    if (m[1]) outcome.deckSearchAnyCardsToHandCount = parseInt(m[1], 10);
    return outcome;
  }

  // 若自己的牌庫的剩餘張數為N張以下，則對手的M隻備戰寶可夢也各受到K點傷害。[...]
  m = t.match(/^若自己的牌庫的剩餘張數為(\d+)張以下，則對手的(\d+)隻備戰寶可夢也各受到(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) {
    if (board.ownDeckCount > parseInt(m[1], 10)) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField), multiTargetOpponentBenchFlatDamage: { count: parseInt(m[2], 10), amount: parseInt(m[3], 10) } };
  }

  // 將雙方的牌庫上方各N張卡丟棄，增加其中能量卡的張數×M點傷害。
  m = t.match(/^將雙方的牌庫上方各(\d+)張卡丟棄，增加其中能量卡的張數×(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), millBothTopScaledDamage: { count: parseInt(m[1], 10), per: parseInt(m[2], 10) } };

  // 從自己的棄牌區選擇N張「X」卡，在給對手看過後加入手牌。
  m = t.match(/^從自己的棄牌區選擇(\d+)張「(.+?)」卡，在給對手看過後加入手牌。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardPileSearchNamedToHandCount: { name: m[2], count: parseInt(m[1], 10) } };

  // 選擇N張這隻寶可夢身上附加的「X」，將其丟棄。這個情況下，將對手的戰鬥寶可夢與附加的卡全部丟棄。
  m = t.match(/^選擇(\d+)張這隻寶可夢身上附加的「(.+?)」，將其丟棄。這個情況下，將對手的戰鬥寶可夢與附加的卡全部丟棄。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardSelfNamedEnergy: { name: m[2], count: parseInt(m[1], 10), thenDiscardDefender: true } };

  // 將這隻寶可夢與附加的卡全部丟棄。(self-discard: NOT a KO — no prizes)
  if (/^將這隻寶可夢與附加的卡，?全部丟棄。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), discardSelfEntirely: true };
  }

  // 擲N次硬幣，在不看手牌正面的情況下，選擇與正面出現的次數相同數量的對手的手牌，將其丟棄。
  m = t.match(/^擲(\d+)次硬幣，在不看手牌正面的情況下，選擇與正面出現的次數相同數量的對手的手牌，將其丟棄。$/);
  if (m) {
    const heads = flipCoins(parseInt(m[1], 10));
    return { baseDamage: parseBaseNumber(damageField), discardRandomOpponentHandCount: heads, coinFlipNote: `${heads}次正面` };
  }

  // 將N個這隻寶可夢身上附加的能量丟棄，對手的M隻備戰寶可夢也受到K點傷害。[...]
  m = t.match(/^將(\d+)個這隻寶可夢身上附加的能量丟棄，對手的(\d+)隻備戰寶可夢(?:也)?(?:各)?受到(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardSelfEnergyCount: parseInt(m[1], 10), multiTargetOpponentBenchFlatDamage: { count: parseInt(m[2], 10), amount: parseInt(m[3], 10) } };

  // 將這隻寶可夢身上附加的(【T】)?能量卡全部丟棄，對手的N隻(備戰)?寶可夢(也)?受到M點傷害。[...]
  m = t.match(/^將這隻寶可夢身上附加的(?:【(.+?)】)?能量卡全部丟棄，對手的(\d+)隻(備戰)?寶可夢(?:也)?(?:各)?受到(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) {
    const dmg = { count: parseInt(m[2], 10), amount: parseInt(m[4], 10) };
    const outcome: GenericAttackOutcome = m[1]
      ? { baseDamage: parseBaseNumber(damageField), discardSelfTypedEnergy: { type: ENERGY_TYPE_FROM_ZH[m[1]] ?? 'Colorless', count: 99 } }
      : { baseDamage: parseBaseNumber(damageField), discardAllSelfEnergy: true };
    if (m[3]) outcome.multiTargetOpponentBenchFlatDamage = dmg;
    else outcome.multiTargetOpponentFlatDamage = dmg;
    return outcome;
  }

  // 若對手的戰鬥寶可夢為進化寶可夢，則增加N點傷害。這個情況下，將這隻寶可夢身上附加的能量卡全部丟棄。
  m = t.match(/^若對手的戰鬥寶可夢為進化寶可夢，則增加(\d+)點傷害。(?:這個情況下，將這隻寶可夢身上附加的能量卡全部丟棄。)?$/);
  if (m) {
    const evolved = board.defenderSubtypes.some(s => s === 'Stage 1' || s === 'Stage 2' || s === 'VMAX' || s === 'VSTAR');
    if (!evolved) return { baseDamage: parseBaseNumber(damageField) };
    const outcome: GenericAttackOutcome = { baseDamage: parseBaseNumber(damageField) + parseInt(m[1], 10) };
    if (t.includes('這個情況下')) outcome.discardAllSelfEnergy = true;
    return outcome;
  }

  // 選擇N隻對手的進化寶可夢，移除N張「進化卡」使其退化。將移除的卡放回對手的手牌。
  m = t.match(/^選擇(\d+)隻對手的進化寶可夢，移除\d+張「進化卡」使其退化。將移除的卡放回對手的手牌。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), devolveOpponentToHandCount: parseInt(m[1], 10) };

  // 若希望，選擇N個對手的戰鬥場的【T階進化】寶可夢身上附加的能量，放回對手的手牌。
  m = t.match(/^若希望，選擇(\d+)個對手的戰鬥場的【(.+?)】寶可夢身上附加的能量，放回對手的手牌。$/);
  if (m) {
    const sub = SUBTYPE_FROM_ZH[m[2]];
    if (sub && !board.defenderSubtypes.includes(sub)) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField), returnOpponentEnergyToHandCount: parseInt(m[1], 10) };
  }

  // 這隻寶可夢以外的雙方的身上放置有傷害指示物的所有寶可夢，各受到N點傷害。[...]
  m = t.match(/^這隻寶可夢以外的雙方的身上放置有傷害指示物的所有寶可夢，各受到(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), damageAllDamagedBothSidesExceptSelf: { amount: parseInt(m[1], 10) } };

  // 造成這隻寶可夢身上附加的【T】能量的數量×N點傷害。若希望，在造成傷害前，從自己的手牌選擇任意數量的「X」卡，附於這隻寶可夢身上。
  m = t.match(/^造成這隻寶可夢身上附加的【(.+?)】能量的數量×(\d+)點傷害。若希望，在造成傷害前，從自己的手牌選擇任意數量的「(.+?)」卡，附於這隻寶可夢身上。$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[1]];
    if (type) return { baseDamage: 0, attachAllNamedFromHandThenTypedScaled: { handName: m[3], type, per: parseInt(m[2], 10) } };
  }

  // 在這隻寶可夢身上放置最多N個傷害指示物，造成放置的數量×M點傷害。
  m = t.match(/^在這隻寶可夢身上放置最多(\d+)個傷害指示物，造成放置的數量×(\d+)點傷害。$/);
  if (m) return { baseDamage: 0, selfCountersScaledDamage: { max: parseInt(m[1], 10), per: parseInt(m[2], 10) } };

  // 增加自己的所有「X」身上附加的【T】能量的數量×N點傷害。
  m = t.match(/^增加自己的所有「(.+?)」身上附加的【(.+?)】能量的數量×(\d+)點傷害。$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[2]];
    if (type) return { baseDamage: parseBaseNumber(damageField), familyTypedEnergyScaledBonus: { family: m[1], type, per: parseInt(m[3], 10) } };
  }

  // 這隻寶可夢受到N點傷害。(recoil, 也-less variant of the existing template)
  m = t.match(/^這隻寶可夢受到(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), selfDamage: parseInt(m[1], 10) };

  // 將對手的戰鬥寶可夢身上附加的「寶可夢道具」卡丟棄。有丟棄的情況下，將對手的戰鬥寶可夢【麻痺】。
  m = t.match(new RegExp(`^將對手的戰鬥寶可夢身上附加的「寶可夢道具」卡丟棄。有丟棄的情況下，將對手的戰鬥寶可夢【(${STATUS_ALT})】。$`));
  if (m) {
    const outcome: GenericAttackOutcome = { baseDamage: parseBaseNumber(damageField), discardOpponentTool: true };
    // The rider fires only when a Tool was actually there — approximated optimistically; the
    // apply site's discardOpponentTool no-ops without one, and statusToInflict is damage-gated.
    outcome.statusToInflict = [STATUS_ZH[m[1]]];
    return outcome;
  }

  // 從自己的棄牌區選擇最多N張基本能量卡，以任意方式附於自己的寶可夢身上。
  m = t.match(/^從自己的棄牌區選擇最多(\d+)張基本能量卡，以任意方式附於自己的寶可夢身上。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardEnergyToOwnBench: { type: '', count: parseInt(m[1], 10) } };

  // 將對手的戰鬥寶可夢【混亂】。因這個【混亂】而放置的傷害指示物的數量改為N個。
  // (Confusion's self-hit is a fixed 30 in this engine — the override infra only models Poison;
  // encode the Confusion itself and accept the flip-damage simplification.)
  m = t.match(/^將對手的戰鬥寶可夢【混亂】。因這個【混亂】而放置的傷害指示物的數量改為(\d+)個。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), statusToInflict: ['Confused'] };

  // 若希望，將場上的競技場卡丟棄。這個情況下，增加N點傷害。
  m = t.match(/^若希望，將場上的競技場卡丟棄。這個情況下，增加(\d+)點傷害。$/);
  if (m) {
    if (!board.hasActiveStadium) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField) + parseInt(m[1], 10), discardActiveStadium: true };
  }

  // 從自己的牌庫選擇「基本【A】能量」卡與「基本【B】能量」卡最多各N張，以任意方式附於自己的寶可夢身上。並且重洗牌庫。
  m = t.match(/^從自己的牌庫選擇「基本【(.+?)】能量」卡與「基本【(.+?)】能量」卡最多各(\d+)張，以任意方式附於自己的寶可夢身上。並且重洗牌庫。$/);
  if (m) {
    const a = ENERGY_TYPE_FROM_ZH[m[1]]; const b = ENERGY_TYPE_FROM_ZH[m[2]];
    if (a && b) {
      // Two typed searches expressed with the one existing signal, run twice via the merge in
      // composeClauses is not possible for a single text — approximate with sequential fields:
      return { baseDamage: parseBaseNumber(damageField), deckSearchNamedEnergyAttachCount: { name: `基本【${m[1]}】能量`, count: parseInt(m[3], 10), benchOnly: false }, deckSearchTypedEnergyToOwnPokemonCount: { type: b, count: parseInt(m[3], 10) } };
    }
  }
  ...
  // 選擇N個這隻寶可夢身上附加的能量，將其丟棄。(bare-count self discard)
  m = t.match(/^選擇(\d+)個這隻寶可夢身上附加的能量，將其丟棄。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardSelfEnergyCount: parseInt(m[1], 10) };

  // 將自己的手牌全部放回牌庫並重洗。然後，從自己的牌庫抽出與對手的手牌張數相同數量的卡。
  if (/^將自己的手牌全部放回牌庫並重洗。然後，從自己的牌庫抽出與對手的手牌張數相同數量的卡。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), shuffleHandThenDrawCount: board.opponentHandCount };
  }

  // 將這隻寶可夢與備戰區的【T】寶可夢互換。
  m = t.match(/^將這隻寶可夢與備戰區的【(.+?)】寶可夢互換。$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[1]];
    if (type) return { baseDamage: parseBaseNumber(damageField), selfSwitchToTypedBench: { type } };
  }

  // 查看自己的牌庫上方N張卡，以任意順序排列，放回牌庫上方。(reorder — random order is a fair pick)
  if (/^查看自己的牌庫上方\d+張卡，以任意順序排列，放回牌庫上方。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField) };
  }

  // 從自己的牌庫選擇最多N張「基本【T】能量」卡，附於這隻寶可夢身上。並且重洗牌庫。附上卡的情況下，將這隻寶可夢【狀態】。
  m = t.match(new RegExp(`^從自己的牌庫選擇最多(\\d+)張「基本【(.+?)】能量」卡，附於這隻寶可夢身上。並且重洗牌庫。附上卡的情況下，將這隻寶可夢【(${STATUS_ALT})】。$`));
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[2]];
    if (type) return { baseDamage: parseBaseNumber(damageField), deckSearchTypedEnergyToSelfCount: { type, count: parseInt(m[1], 10) }, selfStatusToInflict: [STATUS_ZH[m[3]]] };
  }

  // 若這隻寶可夢【狀態】，則增加N點傷害。
  m = t.match(new RegExp(`^若這隻寶可夢【(${STATUS_ALT})】，則增加(\\d+)點傷害。$`));
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.attackerStatusConditions.includes(STATUS_ZH[m[1]]) ? parseInt(m[2], 10) : 0) };

  // 若自己剩餘獎賞卡的張數為N張，則將對手的戰鬥寶可夢【狀態】。
  m = t.match(new RegExp(`^若自己剩餘獎賞卡的張數為(\\d+)張，則將對手的戰鬥寶可夢【(${STATUS_ALT})】。$`));
  if (m) {
    if (board.ownRemainingPrizes !== parseInt(m[1], 10)) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField), statusToInflict: [STATUS_ZH[m[2]]] };
  }

  // 查看對手的手牌，將其中的「物品」卡與「寶可夢道具」卡全部丟棄。
  if (/^查看對手的手牌，將其中的「物品」卡與「寶可夢道具」卡全部丟棄。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), discardOpponentHandItemsAndTools: true };
  }

  // 將這隻寶可夢與備戰寶可夢互換。然後，對手將對手自己的戰鬥寶可夢與備戰寶可夢互換。
  if (/^將這隻寶可夢與備戰寶可夢互換。然後，對手將對手自己的戰鬥寶可夢與備戰寶可夢互換。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), selfSwitchToRandomBench: true, forceOpponentSwitchToRandomBench: true };
  }

  // 對手擲與對手自己的備戰寶可夢的數量相同次數的硬幣。對手的戰鬥寶可夢受到反面出現的次數×N點傷害。這個招式的傷害不計算弱點・抵抗力。
  m = t.match(/^對手擲與對手自己的備戰寶可夢的數量相同次數的硬幣。對手的戰鬥寶可夢受到反面出現的次數×(\d+)點傷害。這個招式的傷害不計算弱點・抵抗力。$/);
  if (m) {
    const tails = board.opponentBenchCount - flipCoins(board.opponentBenchCount);
    return { baseDamage: tails * parseInt(m[1], 10), ignoreWeakness: true, ignoreResistance: true, coinFlipNote: `${tails}次反面` };
  }

  // 從自己的棄牌區選擇N張寶可夢卡，在給對手看過後加入手牌。
  m = t.match(/^從自己的棄牌區選擇(\d+)張寶可夢卡，在給對手看過後加入手牌。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardPileSearchPokemonToHandCount: parseInt(m[1], 10) };

  // 若對手的備戰寶可夢的數量為N隻以上，則增加M點傷害。
  m = t.match(/^若對手的備戰寶可夢的數量為(\d+)隻以上，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.opponentBenchCount >= parseInt(m[1], 10) ? parseInt(m[2], 10) : 0) };

  // 將這隻寶可夢身上附加的能量卡全部丟棄，對手的備戰區的N隻「寶可夢【ex】」受到M點傷害。[...]
  m = t.match(/^將這隻寶可夢身上附加的能量卡全部丟棄，對手的備戰區的(\d+)隻「寶可夢【ex】」受到(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardAllSelfEnergy: true, opponentExBenchFlatDamage: { count: parseInt(m[1], 10), amount: parseInt(m[2], 10) } };

  // 從自己的手牌選擇任意數量的基本能量卡，以任意方式附於自己的寶可夢身上。
  if (/^從自己的手牌選擇任意數量的基本能量卡，以任意方式附於自己的寶可夢身上。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), attachAllBasicEnergyFromHand: true };
  }

  // 從自己的手牌選擇最多N張「X」卡，以任意方式附於自己的寶可夢身上。
  m = t.match(/^從自己的手牌選擇最多(\d+)張「(.+?)」卡，以任意方式附於自己的寶可夢身上。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), attachNamedFromHandCount: { name: m[2], count: parseInt(m[1], 10) } };

  // 擲1次硬幣若為正面，則將對手的戰鬥場的【基礎】寶可夢【昏厥】。若為反面，則選擇1隻對手的備戰區的【基礎】寶可夢，將其【昏厥】。
  if (/^擲1次硬幣若為正面，則將對手的戰鬥場的【基礎】寶可夢【昏厥】。若為反面，則選擇1隻對手的備戰區的【基礎】寶可夢，將其【昏厥】。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), koOpponentBasicCoinSplit: true };
  }

  // 使用這個招式時，若自己剩餘獎賞卡的張數為1張，則這場對戰己方獲勝。
  if (/^使用這個招式時，若自己剩餘獎賞卡的張數為1張，則這場對戰己方獲勝。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), winGameIfOnePrizeLeft: true };
  }

  // 擲1次硬幣若為正面，則將對手的戰鬥寶可夢【狀態】。再選擇N個那隻寶可夢身上附加的能量，將其丟棄。
  m = t.match(new RegExp(`^擲1次硬幣若為正面，則將對手的戰鬥寶可夢【(${STATUS_ALT})】。再選擇(\\d+)個那隻寶可夢身上附加的能量，將其丟棄。$`));
  if (m) {
    const heads = Math.random() < 0.5;
    if (!heads) return { baseDamage: parseBaseNumber(damageField), coinFlipNote: '反面' };
    return { baseDamage: parseBaseNumber(damageField), statusToInflict: [STATUS_ZH[m[1]]], discardOpponentEnergyCount: parseInt(m[2], 10), coinFlipNote: '正面' };
  }

  // 選擇1隻對手的身上放置有N個傷害指示物的寶可夢，將其【昏厥】。
  m = t.match(/^選擇1隻對手的身上放置有(\d+)個傷害指示物的寶可夢，將其【昏厥】。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), koOpponentWithCountersAtLeast: { counters: parseInt(m[1], 10) } };

  // 選擇1個對手的戰鬥寶可夢/戰鬥場的「太晶」寶可夢持有的招式，作為這個招式使用。(also the coin variant via the combinator)
  if (/^選擇1個對手的戰鬥(?:寶可夢|場的「太晶」寶可夢)持有的招式，作為這個招式使用。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), copyDefenderRandomAttack: true };
  }

  // 造成自己的場上的「A」與「B」的數量×N點傷害。自己的場上的所有「A」與「B」也各受到M點傷害。[...]
  m = t.match(/^造成自己的場上的「(.+?)」與「(.+?)」的數量×(\d+)點傷害。自己的場上的所有「.+?」與「.+?」也各受到(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) {
    const n = board.ownFieldNames.filter(x => x.includes(m![1]) || x.includes(m![2])).length;
    return { baseDamage: n * parseInt(m[3], 10), damageOwnFamilyAll: { names: [m[1], m[2]], amount: parseInt(m[4], 10) } };
  }

  // 雙方玩家各從牌庫抽出N張卡。
  m = t.match(/^雙方玩家各從牌庫抽出(\d+)張卡。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), bothDrawCount: parseInt(m[1], 10) };

  // 擲1次硬幣若為反面，則在下個自己的回合，這隻寶可夢無法使用招式。
  if (/^擲1次硬幣若為反面，則在下個自己的回合，這隻寶可夢無法使用招式。$/.test(t)) {
    const heads = Math.random() < 0.5;
    const outcome: GenericAttackOutcome = { baseDamage: parseBaseNumber(damageField), coinFlipNote: heads ? '正面' : '反面' };
    if (!heads) outcome.selfTimedEffect = { kind: 'cantAttack', turnOffset: 2 };
    return outcome;
  }

  // 從自己的牌庫選擇任意數量的【基礎】寶可夢的「X」，放置於備戰區。並且重洗牌庫。
  m = t.match(/^從自己的牌庫選擇任意數量的【基礎】寶可夢的「(.+?)」，放置於備戰區。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchFamilyToBenchCount: { name: m[1].replace(/的寶可夢$/, '的'), count: 99 } };

  // 將這隻寶可夢身上附加的所有能量卡，以任意方式改附於備戰寶可夢身上。
  if (/^將這隻寶可夢身上附加的所有能量卡，以任意方式改附於備戰寶可夢身上。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), moveSelfEnergyToRandomBenchCount: 99 };
  }

  // 若對手的戰鬥寶可夢身上附有「寶可夢道具」卡，則增加N點傷害。
  m = t.match(/^若對手的戰鬥寶可夢身上附有「寶可夢道具」卡，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.defenderHasTool ? parseInt(m[1], 10) : 0) };

  // 選擇N個對手的戰鬥場的「寶可夢【ex】」身上附加的能量，將其丟棄。
  m = t.match(/^選擇(\d+)個對手的戰鬥場的「寶可夢【ex】」身上附加的能量，將其丟棄。$/);
  if (m) {
    if (!board.defenderIsEx) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField), discardOpponentEnergyCount: parseInt(m[1], 10) };
  }

  // 在下個對手的回合，自己的所有「未來」寶可夢不會受到「寶可夢【ex】」招式的傷害。若這隻寶可夢離開戰鬥場，則這個效果消除。
  // (the leave-the-Active cancel is approximated away — one opponent turn either way)
  if (/^在下個對手的回合，自己的所有「未來」寶可夢不會受到「寶可夢【ex】」招式的傷害。(?:若這隻寶可夢離開戰鬥場，則這個效果消除。)?$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), timedImmunityAllOwnFuture: true };
  }

  // 若對手的戰鬥寶可夢身上附有特殊能量卡，則將那隻寶可夢【昏厥】。
  if (/^若對手的戰鬥寶可夢身上附有特殊能量卡，則將那隻寶可夢【昏厥】。$/.test(t)) {
    if (!board.defenderHasSpecialEnergy) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField), koDefender: true };
  }

  // 若場上有競技場卡，則對手的所有備戰寶可夢也各受到N點傷害，並將那張競技場卡丟棄。[...]
  m = t.match(/^若場上有競技場卡，則對手的所有備戰寶可夢也各受到(\d+)點傷害，並將那張競技場卡丟棄。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) {
    if (!board.hasActiveStadium) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField), opponentAllBenchSplashDamage: parseInt(m[1], 10), discardActiveStadium: true };
  }

  // 若自己的手牌與對手的手牌不是相同張數，則這個招式失敗。
  if (/^若自己的手牌與對手的手牌不是相同張數，則這個招式失敗。$/.test(t)) {
    return { baseDamage: board.ownHandCount === board.opponentHandCount ? parseBaseNumber(damageField) : 0 };
  }

  // 在對手的所有寶可夢身上各放置N個傷害指示物。
  m = t.match(/^在對手的所有寶可夢身上各放置(\d+)個傷害指示物。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), placeCountersOnAllOpponent: parseInt(m[1], 10) };

  // 若自己的備戰區沒有「A」「B」，則這個招式失敗。(needs BOTH on the bench)
  m = t.match(/^若自己的備戰區沒有「(.+?)」「(.+?)」，則這個招式失敗。$/);
  if (m) {
    const ok = board.ownBenchNames.some(x => x.includes(m![1])) && board.ownBenchNames.some(x => x.includes(m![2]));
    return { baseDamage: ok ? parseBaseNumber(damageField) : 0 };
  }

  // 若自己的備戰區的「X」身上放置有傷害指示物，則增加N點傷害。
  m = t.match(/^若自己的備戰區的「(.+?)」身上放置有傷害指示物，則增加(\d+)點傷害。$/);
  if (m) {
    const hit = board.ownBenchDamageCountersByName.some(e => e.name.includes(m![1]) && e.counters > 0);
    return { baseDamage: parseBaseNumber(damageField) + (hit ? parseInt(m[2], 10) : 0) };
  }

  // 從自己的牌庫選擇最多N張各不同屬性的基本能量卡，在給對手看過後加入手牌。並且重洗牌庫。
  // (distinct-type constraint auto-picked; the count is what the audit checks)
  m = t.match(/^從自己的牌庫選擇最多(\d+)張各不同屬性的基本能量卡，在給對手看過後加入手牌。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchBasicEnergyToHandCount: parseInt(m[1], 10) };

  // 增加對手的所有寶可夢身上放置的傷害指示物的數量×N點傷害。
  m = t.match(/^增加對手的所有寶可夢身上放置的傷害指示物的數量×(\d+)點傷害。$/);
  if (m) {
    const counters = board.opponentDamageCounters + board.opponentBenchDamageCounters.reduce((s, n) => s + n, 0);
    return { baseDamage: parseBaseNumber(damageField) + counters * parseInt(m[1], 10) };
  }

  // 在下個對手的回合，身上附加的能量為N個以下的所有寶可夢無法使用招式。（包含新上場的寶可夢。）
  m = t.match(/^在下個對手的回合，身上附加的能量為(\d+)個以下的所有寶可夢無法使用招式。（包含新上場的寶可夢。）$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), timedCantAttackAll: { side: 'both', maxEnergy: parseInt(m[1], 10), turnOffset: 1 } };

  // 在下個自己的回合，自己的所有寶可夢無法使用招式。（包含新上場的寶可夢。）
  if (/^在下個自己的回合，自己的所有寶可夢無法使用招式。（包含新上場的寶可夢。）$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), timedCantAttackAll: { side: 'own', turnOffset: 2 } };
  }

  /* ---- Round 4 batch 3 ---- */

  // 選擇N隻自己的備戰寶可夢，將那隻寶可夢與附加的卡全部放回牌庫並重洗。
  m = t.match(/^選擇(\d+)隻自己的備戰寶可夢，將那(?:隻|些)寶可夢與附加的卡，?全部放回牌庫並重洗。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), shuffleOwnBenchToDeckCount: parseInt(m[1], 10) };

  // 從自己的手牌選擇N張能量卡，附於這隻寶可夢身上。(然後，將這隻寶可夢恢復「M」HP。)?
  m = t.match(/^從自己的手牌選擇(\d+)張能量卡，附於這隻寶可夢身上。(?:然後，將這隻寶可夢恢復「(\d+)」HP。)?$/);
  if (m) {
    const outcome: GenericAttackOutcome = { baseDamage: parseBaseNumber(damageField), attachAnyEnergyFromHandToSelfCount: parseInt(m[1], 10) };
    if (m[2]) outcome.healSelfAmount = parseInt(m[2], 10);
    return outcome;
  }

  // 從自己的棄牌區選擇N張基本能量卡，附於備戰寶可夢身上。
  m = t.match(/^從自己的棄牌區選擇(\d+)張基本能量卡，附於備戰寶可夢身上。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardEnergyToOwnBench: { type: '', count: parseInt(m[1], 10) } };

  // 若希望，將最多N張自己的備戰寶可夢身上附加的基本能量卡丟棄，增加其張數×M點傷害。
  // (approximated to the whole own field, the existing signal's scope — documented)
  m = t.match(/^若希望，將最多(\d+)張自己的備戰寶可夢身上附加的基本能量卡丟棄，增加其張數×(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), ownFieldEnergyDiscardScaledDamage: { max: parseInt(m[1], 10), amount: parseInt(m[2], 10) } };

  // 在雙方的所有擁有特性的寶可夢身上，各放置N個傷害指示物。
  m = t.match(/^在雙方的所有擁有特性的寶可夢身上，各放置(\d+)個傷害指示物。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), countersOnAllAbilityHolders: { counters: parseInt(m[1], 10) } };

  // 從自己的牌庫選擇最多N張基本能量卡，以任意方式附於自己的(「未來」)寶可夢身上。並且重洗牌庫。
  m = t.match(/^從自己的牌庫選擇最多(\d+)張基本能量卡，以任意方式附於自己的(?:「.+?」)?寶可夢身上。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchBasicEnergyToOwnPokemonCount: parseInt(m[1], 10) };

  // 擲N次硬幣，造成正面出現的次數×M點傷害。若全部為反面，則將對手的戰鬥寶可夢【狀態】。
  m = t.match(new RegExp(`^擲(\\d+)次硬幣，造成正面出現的次數×(\\d+)點傷害。若全部為反面，則將對手的戰鬥寶可夢【(${STATUS_ALT})】。$`));
  if (m) {
    const heads = flipCoins(parseInt(m[1], 10));
    const outcome: GenericAttackOutcome = { baseDamage: heads * parseInt(m[2], 10), coinFlipNote: `${heads}次正面` };
    if (heads === 0) { outcome.statusToInflict = [STATUS_ZH[m[3]]]; outcome.statusEvenAtZeroDamage = true; }
    return outcome;
  }

  // 從自己的牌庫選擇N張基本能量卡，附於這隻寶可夢身上。並且重洗牌庫。
  m = t.match(/^從自己的牌庫選擇(?:最多)?(\d+)張基本能量卡，附於這隻寶可夢身上。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchBasicEnergyToSelfCount: parseInt(m[1], 10) };

  // 從自己的牌庫選擇N張競技場卡，在給對手看過後加入手牌。並且重洗牌庫。
  if (/^從自己的牌庫選擇1張競技場卡，在給對手看過後加入手牌。並且重洗牌庫。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), deckSearchSupertypeToHand: 'Stadium' };
  }

  // 查看對手的手牌，從其中選擇最多N張【基礎】寶可夢卡，放置於對手的備戰區。
  m = t.match(/^查看對手的手牌，從其中選擇最多(\d+)張【基礎】寶可夢卡，放置於對手的備戰區。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), benchOpponentHandBasicsCount: parseInt(m[1], 10) };

  // 從自己的棄牌區選擇N張訓練家卡，在給對手看過後加入手牌。
  m = t.match(/^從自己的棄牌區選擇(\d+)張訓練家卡，在給對手看過後加入手牌。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardPileSearchTrainerToHandCount: parseInt(m[1], 10) };

  // 若對手的手牌為N張以下，則增加M點傷害。
  m = t.match(/^若對手的手牌為(\d+)張以下，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.opponentHandCount <= parseInt(m[1], 10) ? parseInt(m[2], 10) : 0) };

  // 選擇自己的場上寶可夢身上附加的任意數量的(【T】)?能量卡，以任意方式改附於自己的寶可夢身上。
  // (a pure own-field rearrangement — no auto-play net effect)
  if (/^選擇自己的場上寶可夢身上附加的任意數量的(?:【.+?】)?能量卡，以任意方式改附於自己的寶可夢身上。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField) };
  }

  // 對手的所有寶可夢(也)?各受到N點傷害。[...]
  m = t.match(/^對手的所有寶可夢(?:也)?各受到(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), multiTargetOpponentFlatDamage: { count: 99, amount: parseInt(m[1], 10) } };

  // 對手的所有備戰寶可夢(也)?各受到N點傷害。[...] (也-less variant)
  m = t.match(/^對手的所有備戰寶可夢(?:也)?各受到(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), opponentAllBenchSplashDamage: parseInt(m[1], 10) };

  // 自己的所有備戰寶可夢也各受到N點傷害。 [...]
  m = t.match(/^自己的所有備戰寶可夢(?:也)?各受到(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), selfAllBenchSplashDamage: parseInt(m[1], 10) };

  // 在對手的所有備戰寶可夢身上放置傷害指示物直到各自的剩餘HP變為「N」為止。
  m = t.match(/^在對手的所有備戰寶可夢身上放置傷害指示物直到各自的剩餘HP變為「(\d+)」為止。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), setAllOpponentBenchRemainingHp: parseInt(m[1], 10) };

  // 造成對手的所有寶可夢身上附加的特殊能量卡的張數×N點傷害。
  m = t.match(/^造成對手的所有寶可夢身上附加的特殊能量卡的張數×(\d+)點傷害。$/);
  if (m) return { baseDamage: board.opponentFieldSpecialEnergyCount * parseInt(m[1], 10) };

  // 造成這隻寶可夢身上附加的特殊能量卡的張數×N點傷害。
  m = t.match(/^造成這隻寶可夢身上附加的特殊能量卡的張數×(\d+)點傷害。$/);
  if (m) return { baseDamage: board.attackerSpecialEnergyCount * parseInt(m[1], 10) };

  // 若自己的所有備戰寶可夢身上都放置有傷害指示物，則增加N點傷害。
  m = t.match(/^若自己的所有備戰寶可夢身上都放置有傷害指示物，則增加(\d+)點傷害。$/);
  if (m) {
    const all = board.ownBenchCount > 0 && board.ownDamagedBenchCount === board.ownBenchCount;
    return { baseDamage: parseBaseNumber(damageField) + (all ? parseInt(m[1], 10) : 0) };
  }

  // 若這隻寶可夢處於特殊狀態，則使用這個招式所需的能量就算沒有附於這隻寶可夢身上，這個招式也可使用。
  // / 若這隻寶可夢身上放置有傷害指示物，則這個招式只需要N個【T】能量即可使用。
  // — cost-waiver texts; the actual cost change lives in canAttack (which reads the printed text).
  if (/^若這隻寶可夢處於特殊狀態，則使用這個招式所需的能量就算沒有附於這隻寶可夢身上，這個招式也可使用。$/.test(t)
    || /^若這隻寶可夢身上放置有傷害指示物，則這個招式只需要\d+個【.+?】能量即可使用。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField) };
  }

  // 將雙方的戰鬥寶可夢【昏厥】。
  if (/^將雙方的戰鬥寶可夢【昏厥】。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), koBothActives: true };
  }

  // 若這隻寶可夢身上附加的能量的數量，比對手的戰鬥寶可夢身上附加的能量的數量多，則增加N點傷害。
  m = t.match(/^若這隻寶可夢身上附加的能量的數量，比對手的戰鬥寶可夢身上附加的能量的數量多，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.attackerTotalEnergyCount > board.opponentEnergyCount ? parseInt(m[1], 10) : 0) };

  // 在下個對手的回合，受到這個招式的【基礎】寶可夢，無法使用招式。
  if (/^在下個對手的回合，受到這個招式的【基礎】寶可夢，無法使用招式。$/.test(t)) {
    if (!board.defenderSubtypes.includes('Basic')) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField), opponentTimedEffect: { kind: 'cantAttack', turnOffset: 1 } };
  }

  // 將對手的戰鬥寶可夢與備戰寶可夢互換（由對手選擇…）。然後，新上場的寶可夢受到N點傷害。
  m = t.match(/^將對手的戰鬥寶可夢與備戰寶可夢互換（由對手選擇放置於戰鬥場的寶可夢）。然後，新上場的寶可夢受到(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), forceOpponentSwitchToRandomBench: true, splashDamageAfterSwitch: parseInt(m[1], 10) };

  // 在造成傷害前，將這隻寶可夢身上附加的「寶可夢道具」卡丟棄。若無法丟棄，則這個招式失敗。
  if (/^在造成傷害前，將這隻寶可夢身上附加的「寶可夢道具」卡丟棄。若無法丟棄，則這個招式失敗。$/.test(t)) {
    if (!board.attackerHasTool) return { baseDamage: 0 };
    return { baseDamage: parseBaseNumber(damageField), discardSelfTool: true };
  }

  // 將N個傷害指示物以任意方式放置於對手的備戰寶可夢身上。
  m = t.match(/^將(\d+)個傷害指示物以任意方式放置於對手的備戰寶可夢身上。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), placeCountersOnRandomOpponentBench: parseInt(m[1], 10) };

  // 從自己的棄牌區選擇N張「基本【T】能量」卡，附於自己的【T】寶可夢身上。(holder restriction auto-picked)
  m = t.match(/^從自己的棄牌區選擇(\d+)張「基本【(.+?)】能量」卡，附於自己的(?:【.+?】)?寶可夢身上。$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[2]];
    if (type) return { baseDamage: parseBaseNumber(damageField), discardEnergyToOwnBench: { type, count: parseInt(m[1], 10) } };
  }

  // 從對手的棄牌區選擇最多N張能量卡，以任意方式附於對手的寶可夢身上。
  m = t.match(/^從對手的棄牌區選擇最多(\d+)張能量卡，以任意方式附於對手的寶可夢身上。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), attachOpponentDiscardEnergyToTheirPokemonCount: parseInt(m[1], 10) };

  // 在下個對手的回合，這隻寶可夢受到招式的傷害時，將與受到的傷害相同數值的傷害指示物，放置於使用招式的寶可夢身上。
  if (/^在下個對手的回合，這隻寶可夢受到招式的傷害時，將與受到的傷害相同數值的傷害指示物，放置於使用招式的寶可夢身上。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), selfTimedEffect: { kind: 'retaliationMirror', turnOffset: 1 } };
  }

  // 從棄牌區附給自己的所有備戰寶可夢各N張「基本【T】能量」卡。
  m = t.match(/^從棄牌區附給自己的所有備戰寶可夢各1張「基本【(.+?)】能量」卡。$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[1]];
    if (type) return { baseDamage: parseBaseNumber(damageField), discardPileTypedEnergyToAllBenchEach: type };
  }

  // 選擇最多N張對手的場上寶可夢身上附加的「寶可夢道具」卡，將其丟棄。
  m = t.match(/^選擇最多(\d+)張對手的場上寶可夢身上附加的「寶可夢道具」卡，將其丟棄。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardOpponentFieldToolsCount: parseInt(m[1], 10) };

  // 將雙方的戰鬥寶可夢【睡眠】。在下個自己的回合，這隻寶可夢使用的招式，對對手的戰鬥寶可夢造成的傷害「+N」點。
  m = t.match(/^將雙方的戰鬥寶可夢【睡眠】。在下個自己的回合，這隻寶可夢使用的招式，對對手的戰鬥寶可夢造成的傷害「\+(\d+)」點。$/);
  if (m) {
    return {
      baseDamage: parseBaseNumber(damageField), statusToInflict: ['Asleep'], statusEvenAtZeroDamage: true,
      selfStatusToInflict: ['Asleep'], selfTimedEffect: { kind: 'outgoingDamageBoost', amount: parseInt(m[1], 10), turnOffset: 2 },
    };
  }

  // 從自己的手牌選擇N張基本能量卡，附於自己的寶可夢身上。
  m = t.match(/^從自己的手牌選擇(\d+)張基本能量卡，附於自己的寶可夢身上。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), attachNamedFromHandCount: { name: '基本', count: parseInt(m[1], 10) } };

  // 擲3次硬幣。若出現1次正面，則增加A點傷害。若出現2次正面，則增加B點傷害。若全部為正面，則增加C點傷害。
  m = t.match(/^擲3次硬幣。若出現1次正面，則增加(\d+)點傷害。若出現2次正面，則增加(\d+)點傷害。若全部為正面，則增加(\d+)點傷害。$/);
  if (m) {
    const heads = flipCoins(3);
    const bonus = heads === 1 ? parseInt(m[1], 10) : heads === 2 ? parseInt(m[2], 10) : heads === 3 ? parseInt(m[3], 10) : 0;
    return { baseDamage: parseBaseNumber(damageField) + bonus, coinFlipNote: `${heads}次正面` };
  }

  // 若對手的戰鬥寶可夢【撤退】所需的能量為N個以上，則增加M點傷害。
  m = t.match(/^若對手的戰鬥寶可夢【撤退】所需的能量為(\d+)個以上，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.defenderRetreatCost >= parseInt(m[1], 10) ? parseInt(m[2], 10) : 0) };

  // 從自己的棄牌區選擇最多N張「X」卡，在給對手看過後放回牌庫並重洗。
  m = t.match(/^從自己的棄牌區選擇最多(\d+)張「(.+?)」卡，在給對手看過後放回牌庫並重洗。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardPileNamedToDeckCount: { name: m[2], count: parseInt(m[1], 10) } };

  // 從自己的手牌將最多N張能量卡丟棄，對對手的1隻寶可夢，造成其張數×M點傷害。[...]
  // (approximated to the defender-targeted hand-discard-scaled signal — documented)
  m = t.match(/^從自己的手牌將最多(\d+)張能量卡丟棄，對對手的1隻寶可夢，造成其張數×(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), handDiscardScaledDamage: { filter: { kind: 'anyEnergy' }, max: parseInt(m[1], 10), amount: parseInt(m[2], 10) } };

  /* ---- Round 4 batch 4 ---- */

  // 在上個自己的回合，若這隻寶可夢使出了/使用了「X」，則增加N點傷害。
  m = t.match(/^在上個自己的回合，若這隻寶可夢使(?:出|用)了「[‌​]*(.+?)」，則增加(\d+)點傷害。$/);
  if (m) {
    const used = board.attackerAttacksUsedLastTurn.some(n => n.replace(/^[‌​\s]+/, '') === m![1]);
    return { baseDamage: parseBaseNumber(damageField) + (used ? parseInt(m[2], 10) : 0) };
  }

  // 在上個自己的回合，若這隻寶可夢以外的「古代」寶可夢使用了招式，則增加N點傷害。
  m = t.match(/^在上個自己的回合，若這隻寶可夢以外的「古代」寶可夢使用了招式，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.ownOtherAncientAttackedLastTurn ? parseInt(m[1], 10) : 0) };

  // 在上個自己的回合，若自己的寶可夢使用了「X」，則無法使用這個招式。(the ban is a canAttack gate)
  if (/^在上個自己的回合，若自己的寶可夢使用了「.+?」，則無法使用這個招式。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField) };
  }

  // 在這個回合，若從手牌使出了「X」，則增加N點傷害。(exact Supporter name)
  m = t.match(/^在這個回合，若從手牌使出了「(.+?)」，則增加(\d+)點傷害。$/);
  if (m && !['古代', '未來'].includes(m[1])) {
    const played = board.ownSupporterNamesPlayedThisTurn.some(n => n.includes(m![1]));
    return { baseDamage: parseBaseNumber(damageField) + (played ? parseInt(m[2], 10) : 0) };
  }

  // 若對手剩餘獎賞卡的張數不是N張・M張，則這個招式失敗。
  m = t.match(/^若對手剩餘獎賞卡的張數不是(\d+)張・(\d+)張，則這個招式失敗。$/);
  if (m) {
    const ok = board.opponentRemainingPrizes === parseInt(m[1], 10) || board.opponentRemainingPrizes === parseInt(m[2], 10);
    return { baseDamage: ok ? parseBaseNumber(damageField) : 0 };
  }

  // 將自己的牌庫上方N張卡丟棄，造成其中能量卡的張數×M點傷害。
  m = t.match(/^將自己的牌庫上方(\d+)張卡丟棄，造成其中能量卡的張數×(\d+)點傷害。$/);
  if (m) return { baseDamage: 0, selfMillEnergyScaledDamage: { count: parseInt(m[1], 10), per: parseInt(m[2], 10) } };

  // 這個招式在後攻玩家的最初回合無法使用。造成自己的備戰寶可夢的數量×N點傷害。(ban in canAttack)
  m = t.match(/^這個招式在後攻玩家的最初回合無法使用。造成自己的備戰寶可夢的數量×(\d+)點傷害。$/);
  if (m) return { baseDamage: board.ownBenchCount * parseInt(m[1], 10) };

  // 在下個對手的回合，這隻寶可夢不會受到【基礎】寶可夢(（【無】寶可夢除外）)?招式的傷害。
  // (the Colorless carve-out is dropped — documented approximation)
  if (/^在下個對手的回合，這隻寶可夢不會受到【基礎】寶可夢(?:（【無】寶可夢除外）)?招式的傷害。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), selfTimedEffect: { kind: 'damageImmune', vsSubtype: 'Basic', turnOffset: 1 } };
  }

  // 在下個對手的回合，這隻寶可夢不會受到進化寶可夢招式的傷害。
  if (/^在下個對手的回合，這隻寶可夢不會受到進化寶可夢招式的傷害。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), selfTimedEffect: { kind: 'damageImmune', vsSubtype: 'Evolved', turnOffset: 1 } };
  }

  // 在下個對手的回合，這隻寶可夢不會受到擁有特性的寶可夢招式的傷害。
  if (/^在下個對手的回合，這隻寶可夢不會受到擁有特性的寶可夢招式的傷害。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), selfTimedEffect: { kind: 'damageImmune', vsSubtype: 'HasAbility', turnOffset: 1 } };
  }

  // 將自己的場上寶可夢身上附加的任意數量的基本能量卡丟棄，造成其張數×N點傷害。
  m = t.match(/^將自己的場上寶可夢身上附加的任意數量的基本能量卡丟棄，造成其張數×(\d+)點傷害。$/);
  if (m) return { baseDamage: 0, ownFieldEnergyDiscardScaledDamage: { amount: parseInt(m[1], 10) } };

  // 從自己的牌庫選擇最多N張能量卡，在給對手看過後加入手牌。並且重洗牌庫。
  m = t.match(/^從自己的牌庫選擇最多(\d+)張能量卡，在給對手看過後加入手牌。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchAnyEnergyToHandCount: parseInt(m[1], 10) };

  // 將這隻寶可夢身上附加的能量卡全部丟棄，獲得1張自己的獎賞卡。
  if (/^將這隻寶可夢身上附加的能量卡全部丟棄，獲得1張自己的獎賞卡。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), discardAllSelfEnergy: true, takeOwnPrizeCount: 1 };
  }

  // 增加自己的棄牌區的「古代」卡的張數×N點傷害。
  m = t.match(/^增加自己的棄牌區的「古代」卡的張數×(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + board.ownDiscardAncientCount * parseInt(m[1], 10) };

  // 從自己的牌庫選擇N張能量卡，附於這隻寶可夢/備戰寶可夢身上。並且重洗牌庫。
  m = t.match(/^從自己的牌庫選擇(\d+)張能量卡，附於(這隻寶可夢|備戰寶可夢)身上。並且重洗牌庫。$/);
  if (m) {
    if (m[2] === '這隻寶可夢') return { baseDamage: parseBaseNumber(damageField), deckSearchAnyEnergyToSelfCount: parseInt(m[1], 10) };
    return { baseDamage: parseBaseNumber(damageField), deckSearchNamedEnergyAttachCount: { name: '', count: parseInt(m[1], 10), benchOnly: true } };
  }

  // 雙方玩家各自將手牌全部放回牌庫並重洗。然後，各自從牌庫抽出N張卡。
  m = t.match(/^雙方玩家各自將手牌全部放回牌庫並重洗。然後，各自從牌庫抽出(\d+)張卡。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), bothShuffleHandDrawCount: parseInt(m[1], 10) };

  // 將對手的牌庫上方N張卡翻到正面，從其中選擇任意數量的【基礎】寶可夢卡，放置於對手的備戰區。將剩餘卡放回牌庫並重洗。
  m = t.match(/^將對手的牌庫上方(\d+)張卡翻到正面，從其中選擇任意數量的【基礎】寶可夢卡，放置於對手的備戰區。將剩餘卡放回牌庫並重洗。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), revealOpponentTopBenchBasicsCount: parseInt(m[1], 10) };

  // 選擇對手的N隻備戰寶可夢，與戰鬥寶可夢互換。然後，新上場的寶可夢受到M點傷害。
  m = t.match(/^選擇對手的1隻備戰寶可夢，與戰鬥寶可夢互換。然後，新上場的寶可夢受到(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), gustOpponentBenchThenSplash: { splash: parseInt(m[1], 10) } };

  // 從自己的手牌將任意數量的寶可夢卡給對手看過後，造成給對手看過的寶可夢的屬性種類的數量×N點傷害。
  m = t.match(/^從自己的手牌將任意數量的寶可夢卡給對手看過後，造成給對手看過的寶可夢的屬性種類的數量×(\d+)點傷害。$/);
  if (m) return { baseDamage: board.ownHandPokemonTypeCount * parseInt(m[1], 10) };

  // 對手的1隻寶可夢受到這隻寶可夢身上附加的【T】能量的數量×N點傷害。[...]
  m = t.match(/^對手的1隻寶可夢受到這隻寶可夢身上附加的【(.+?)】能量的數量×(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[1]];
    if (type) {
      const n = board.attackerEnergyCounts[type] ?? 0;
      return { baseDamage: parseBaseNumber(damageField), multiTargetOpponentFlatDamage: { count: 1, amount: n * parseInt(m[2], 10) } };
    }
  }

  // 將這隻寶可夢與附加的卡全部放回牌庫並重洗。(自己的-less variant)
  if (/^將這隻寶可夢與附加的卡，?全部放回牌庫並重洗。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), returnSelfAndAttachmentsToDeck: true };
  }

  // 若對手剩餘獎賞卡的張數為N張，則增加M點傷害。
  m = t.match(/^若對手剩餘獎賞卡的張數為(\d+)張，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.opponentRemainingPrizes === parseInt(m[1], 10) ? parseInt(m[2], 10) : 0) };

  // 選擇任意數量的自己的手牌，將其丟棄。(auto-plays as "discard 0")
  if (/^選擇任意數量的自己的手牌，將其丟棄。$/.test(t)) return { baseDamage: parseBaseNumber(damageField) };

  // 若自己1張手牌都沒有，則增加N點傷害。
  m = t.match(/^若自己1張手牌都沒有，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.ownHandCount === 0 ? parseInt(m[1], 10) : 0) };

  // 在對手的N隻備戰寶可夢身上放置M個傷害指示物。
  m = t.match(/^在對手的1隻備戰寶可夢身上放置(\d+)個傷害指示物。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), placeCountersOnRandomOpponentBench: parseInt(m[1], 10) };

  // 對手的1隻身上附有特殊能量卡的寶可夢受到N點傷害。[...]
  m = t.match(/^對手的1隻身上附有特殊能量卡的寶可夢受到(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), opponentSpecialEnergyHolderSplash: { amount: parseInt(m[1], 10) } };

  // 若這隻寶可夢身上放置有N個以上的傷害指示物，則增加M點傷害。(這個招式的傷害不計算弱點。)?
  m = t.match(/^若這隻寶可夢身上放置有(\d+)個以上的傷害指示物，則增加(\d+)點傷害。(這個招式的傷害不計算弱點。)?$/);
  if (m) {
    const outcome: GenericAttackOutcome = { baseDamage: parseBaseNumber(damageField) + (board.selfDamageCounters >= parseInt(m[1], 10) ? parseInt(m[2], 10) : 0) };
    if (m[3]) outcome.ignoreWeakness = true;
    return outcome;
  }

  // 擲N次硬幣，若全部為正面，則從自己的牌庫選擇1張寶可夢卡，放置於備戰區。並且重洗牌庫。
  m = t.match(/^擲(\d+)次硬幣，若全部為正面，則從自己的牌庫選擇(\d+)張寶可夢卡，放置於備戰區。並且重洗牌庫。$/);
  if (m) {
    const flips = parseInt(m[1], 10);
    const heads = flipCoins(flips);
    if (heads < flips) return { baseDamage: parseBaseNumber(damageField), coinFlipNote: `${heads}次正面` };
    return { baseDamage: parseBaseNumber(damageField), deckSearchPokemonToBenchCount: parseInt(m[2], 10), coinFlipNote: '全部正面' };
  }

  // 造成自己的場上的「X（包含…）」的數量×N點傷害。(the parenthetical is a reminder, not a filter)
  m = t.match(/^造成自己的場上的「(.+?)（.+?）」的數量×(\d+)點傷害。$/);
  if (m) return { baseDamage: 0, familyScaledDamage: { name: m[1], amount: parseInt(m[2], 10) } };

  // 從自己的棄牌區選擇最多N張【基礎】寶可夢卡，放置於備戰區。
  m = t.match(/^從自己的棄牌區選擇最多(\d+)張【基礎】寶可夢卡，放置於備戰區。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardPileSearchBasicToBenchCount: parseInt(m[1], 10) };

  // 選擇N隻自己的備戰寶可夢，將所選的寶可夢身上放置的傷害指示物，全部改放於對手的戰鬥寶可夢身上。
  m = t.match(/^選擇(\d+)隻自己的備戰寶可夢，將所選的寶可夢身上放置的傷害指示物，全部改放於對手的戰鬥寶可夢身上。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), moveNamedBenchDamageToDefender: { name: '', count: parseInt(m[1], 10) } };

  // 造成雙方的所有寶可夢身上附加的能量的數量×N點傷害。
  m = t.match(/^造成雙方的所有寶可夢身上附加的能量的數量×(\d+)點傷害。$/);
  ...
  m = t.match(/^雙方玩家若希望，各自從自己的手牌選擇最多(\d+)張基本能量卡，以任意方式附於自己的寶可夢身上。（對手先選擇。）$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), bothAttachHandBasicsCount: parseInt(m[1], 10) };

  // 若身上附有的能量比使用這個招式所需的能量多N個，則對手的M隻備戰寶可夢也受到K點傷害。[...]
  m = t.match(/^若身上附有的能量比使用這個招式所需的能量多(\d+)個，則對手的(\d+)隻備戰寶可夢也受到(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) {
    if (board.attackerTotalEnergyCount < board.attackCostCount + parseInt(m[1], 10)) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField), multiTargetOpponentBenchFlatDamage: { count: parseInt(m[2], 10), amount: parseInt(m[3], 10) } };
  }

  // 將雙方的所有寶可夢各恢復「N」HP。
  m = t.match(/^將雙方的所有寶可夢各恢復「(\d+)」HP。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), healAllBothSidesAmount: parseInt(m[1], 10) };

  // 在下個對手的回合，受到這個招式的寶可夢，無法附上從手牌使出的能量卡。
  if (/^在下個對手的回合，受到這個招式的寶可夢，無法附上從手牌使出的能量卡。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), opponentTimedEffect: { kind: 'cantAttachEnergy', turnOffset: 1 } };
  }

  // 從對手的進化的戰鬥寶可夢身上，移除N張「進化卡」使其退化。將移除的卡放回對手的手牌。
  m = t.match(/^從對手的進化的戰鬥寶可夢身上，移除(\d+)張「進化卡」使其退化。將移除的卡放回對手的手牌。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), devolveOpponentToHandCount: parseInt(m[1], 10) };

  // 若這隻寶可夢身上附有特殊能量卡，則增加N點傷害。
  m = t.match(/^若這隻寶可夢身上附有特殊能量卡，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.attackerSpecialEnergyCount > 0 ? parseInt(m[1], 10) : 0) };

  // 將對手的所有寶可夢身上附加的「寶可夢道具」卡與「特殊能量」卡全部丟棄。
  if (/^將對手的所有寶可夢身上附加的「寶可夢道具」卡與「特殊能量」卡全部丟棄。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), discardAllOpponentFieldSpecialEnergy: true, discardOpponentFieldToolsCount: 99 };
  }

  // 選擇1隻對手的寶可夢，擲與自己的場上的，名稱中有「X」的寶可夢的數量相同次數的硬幣。所選的寶可夢受到正面出現的次數×N點傷害。[...]
  m = t.match(/^選擇1隻對手的寶可夢，擲與自己的場上的，名稱中有「(.+?)」的寶可夢的數量相同次數的硬幣。所選的寶可夢受到正面出現的次數×(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) {
    const flips = board.ownFieldNames.filter(x => x.includes(m![1])).length;
    const heads = flipCoins(flips);
    return { baseDamage: parseBaseNumber(damageField), multiTargetOpponentFlatDamage: { count: 1, amount: heads * parseInt(m[2], 10) }, coinFlipNote: `${heads}次正面` };
  }

  // 擲N次硬幣。若出現正面，則查看對手的手牌，從其中選擇與正面出現的次數相同數量的卡，放回對手的牌庫並重洗。
  m = t.match(/^擲(\d+)次硬幣。若出現正面，則查看對手的手牌，從其中選擇與正面出現的次數相同數量的卡，放回對手的牌庫並重洗。$/);
  if (m) {
    const heads = flipCoins(parseInt(m[1], 10));
    return { baseDamage: parseBaseNumber(damageField), shuffleRandomOpponentHandCardsIntoDeckCount: heads, coinFlipNote: `${heads}次正面` };
  }

  /* ---- Round 4 batch 5 ---- */

  // 從自己的牌庫選擇N張能量卡，附於備戰區的【T】寶可夢身上。並且重洗牌庫。(typed holder auto-picked)
  m = t.match(/^從自己的牌庫選擇(\d+)張能量卡，附於備戰區的【.+?】寶可夢身上。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchNamedEnergyAttachCount: { name: '', count: parseInt(m[1], 10), benchOnly: true } };

  // 從自己的牌庫任意選擇最多N張卡加入手牌。並且重洗牌庫。(bare variant of the 若希望 template)
  m = t.match(/^從自己的牌庫任意選擇最多(\d+)張卡加入手牌。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchAnyCardsToHandCount: parseInt(m[1], 10) };

  // 從自己的棄牌區選擇最多與對手的所有寶可夢身上附加的能量的數量相同數量的「基本【T】能量」卡，以任意方式附於自己的【T】寶可夢身上。
  m = t.match(/^從自己的棄牌區選擇最多與對手的所有寶可夢身上附加的能量的數量相同數量的「基本【(.+?)】能量」卡，以任意方式附於自己的(?:【.+?】)?寶可夢身上。$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[1]];
    if (type) return { baseDamage: parseBaseNumber(damageField), discardEnergyToOwnBench: { type, count: board.opponentFieldTotalEnergyCount } };
  }

  // 從自己的手牌將最多N張能量卡丟棄，增加其張數×M點傷害。(增加 variant)
  m = t.match(/^從自己的手牌將最多(\d+)張能量卡丟棄，增加其張數×(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), handDiscardScaledDamage: { filter: { kind: 'anyEnergy' }, max: parseInt(m[1], 10), amount: parseInt(m[2], 10) } };

  // 將對手所有剩餘HP為「N」以下的寶可夢【昏厥】。
  m = t.match(/^將對手所有剩餘HP為「(\d+)」以下的寶可夢【昏厥】。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), koAllOpponentRemainingHpAtMost: parseInt(m[1], 10) };

  // 從自己的手牌將任意數量的「A」「B」「C」給對手看過後，造成其張數×N點傷害。
  m = t.match(/^從自己的手牌將任意數量的「(.+?)」「(.+?)」(?:「(.+?)」)?給對手看過後，造成其張數×(\d+)點傷害。$/);
  if (m) {
    const names = [m[1], m[2], m[3]].filter((x): x is string => !!x);
    const n = board.ownHandNames.filter(x => names.some(nm => x.includes(nm))).length;
    return { baseDamage: n * parseInt(m[4], 10) };
  }

  // 擲硬幣直到出現反面，從自己的牌庫選擇最多與正面出現的次數相同數量的基本能量卡，附於這隻寶可夢身上。並且重洗牌庫。
  if (/^擲硬幣直到出現反面，從自己的牌庫選擇最多與正面出現的次數相同數量的基本能量卡，附於這隻寶可夢身上。並且重洗牌庫。$/.test(t)) {
    let heads = 0;
    while (Math.random() < 0.5) heads++;
    return { baseDamage: parseBaseNumber(damageField), deckSearchBasicEnergyToSelfCount: heads, coinFlipNote: `${heads}次正面` };
  }

  // 在不看正面的情況下，將對手的手牌丟棄直到張數變為N張為止。
  m = t.match(/^在不看正面的情況下，將對手的手牌丟棄直到張數變為(\d+)張為止。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardOpponentHandDownTo: parseInt(m[1], 10) };

  // 從自己的牌庫選擇最多N張抵抗力為【T】屬性的寶可夢卡，在給對手看過後加入手牌。並且重洗牌庫。
  // (the resistance filter is auto-picked — documented approximation)
  m = t.match(/^從自己的牌庫選擇最多(\d+)張抵抗力為【.+?】屬性的寶可夢卡，在給對手看過後加入手牌。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchPokemonToHandCount: parseInt(m[1], 10) };

  // 將N個這隻寶可夢身上附加的能量丟棄，對手的M隻寶可夢受到K點傷害。[...] (any-position variant)
  m = t.match(/^將(\d+)個這隻寶可夢身上附加的能量丟棄，對手的(\d+)隻寶可夢(?:也)?(?:各)?受到(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardSelfEnergyCount: parseInt(m[1], 10), multiTargetOpponentFlatDamage: { count: parseInt(m[2], 10), amount: parseInt(m[3], 10) } };

  // 從自己的棄牌區選擇最多N張基本能量卡，附於1隻備戰寶可夢身上。
  m = t.match(/^從自己的棄牌區選擇最多(\d+)張基本能量卡，附於1隻備戰寶可夢身上。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardEnergyToOwnBench: { type: '', count: parseInt(m[1], 10) } };

  // 選擇N個對手的戰鬥寶可夢身上附加的【T】能量，將其丟棄。
  m = t.match(/^選擇(\d+)個對手的戰鬥寶可夢身上附加的【(.+?)】能量，將其丟棄。$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[2]];
    if (type) return { baseDamage: parseBaseNumber(damageField), discardOpponentTypedEnergy: { type, count: parseInt(m[1], 10) } };
  }

  // 在下個對手的回合，每次對手從手牌將能量卡附於受到這個招式的寶可夢身上時，在那隻寶可夢身上放置N個傷害指示物。
  m = t.match(/^在下個對手的回合，每次對手從手牌將能量卡附於受到這個招式的寶可夢身上時，在那隻寶可夢身上放置(\d+)個傷害指示物。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), opponentTimedEffect: { kind: 'attachPunishCounters', amount: parseInt(m[1], 10), turnOffset: 1 } };

  // 選擇1隻自己的備戰寶可夢，將所選的寶可夢身上放置的傷害指示物，全部改放於對手的1隻寶可夢身上。
  // (target approximated to the defender — documented)
  if (/^選擇1隻自己的備戰寶可夢，將所選的寶可夢身上放置的傷害指示物，全部改放於對手的1隻寶可夢身上。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), moveNamedBenchDamageToDefender: { name: '', count: 1 } };
  }

  // 若自己的手牌與對手的手牌張數相同，則增加N點傷害。
  m = t.match(/^若自己的手牌與對手的手牌張數相同，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.ownHandCount === board.opponentHandCount ? parseInt(m[1], 10) : 0) };

  // 擲硬幣直到出現反面，在不看手牌正面的情況下，從對手的手牌選擇與正面出現的次數相同數量的卡，查看那些卡的正面後放回對手的牌庫並重洗。
  if (/^擲硬幣直到出現反面，在不看手牌正面的情況下，從對手的手牌選擇與正面出現的次數相同數量的卡，查看那些卡的正面後放回對手的牌庫並重洗。$/.test(t)) {
    let heads = 0;
    while (Math.random() < 0.5) heads++;
    return { baseDamage: parseBaseNumber(damageField), shuffleRandomOpponentHandCardsIntoDeckCount: heads, coinFlipNote: `${heads}次正面` };
  }

  // 減少對手的戰鬥寶可夢身上附加的能量的數量×N點傷害。
  m = t.match(/^減少對手的戰鬥寶可夢身上附加的能量的數量×(\d+)點傷害。$/);
  if (m) return { baseDamage: Math.max(0, parseBaseNumber(damageField) - board.opponentEnergyCount * parseInt(m[1], 10)) };

  // 擲硬幣直到出現反面，從自己的牌庫任意選擇最多與正面出現的次數相同數量的卡，加入手牌。並且重洗牌庫。
  if (/^擲硬幣直到出現反面，從自己的牌庫任意選擇最多與正面出現的次數相同數量的卡，加入手牌。並且重洗牌庫。$/.test(t)) {
    let heads = 0;
    while (Math.random() < 0.5) heads++;
    return { baseDamage: parseBaseNumber(damageField), deckSearchAnyCardsToHandCount: heads, coinFlipNote: `${heads}次正面` };
  }

  // 擲N次硬幣，若全部為正面，則將自己的1隻寶可夢的HP全部恢復。
  m = t.match(/^擲(\d+)次硬幣，若全部為正面，則將自己的1隻寶可夢的HP全部恢復。$/);
  if (m) {
    const flips = parseInt(m[1], 10);
    const heads = flipCoins(flips);
    if (heads < flips) return { baseDamage: parseBaseNumber(damageField), coinFlipNote: `${heads}次正面` };
    return { baseDamage: parseBaseNumber(damageField), healOneOwnFullHeal: { benchOnly: false }, coinFlipNote: '全部正面' };
  }

  // 若這隻寶可夢【A】或者【B】，則增加N點傷害。
  m = t.match(new RegExp(`^若這隻寶可夢【(${STATUS_ALT})】或者【(${STATUS_ALT})】，則增加(\\d+)點傷害。$`));
  if (m) {
    const hit = board.attackerStatusConditions.includes(STATUS_ZH[m[1]]) || board.attackerStatusConditions.includes(STATUS_ZH[m[2]]);
    return { baseDamage: parseBaseNumber(damageField) + (hit ? parseInt(m[3], 10) : 0) };
  }

  // 擲N次硬幣，從自己的牌庫選擇最多與正面出現的次數×M張相同數量的基本能量卡，以任意方式附於自己的寶可夢身上。並且重洗牌庫。
  m = t.match(/^擲(\d+)次硬幣，從自己的牌庫選擇最多與正面出現的次數×(\d+)張相同數量的基本能量卡，以任意方式附於自己的寶可夢身上。並且重洗牌庫。$/);
  if (m) {
    const heads = flipCoins(parseInt(m[1], 10));
    return { baseDamage: parseBaseNumber(damageField), deckSearchBasicEnergyToOwnPokemonCount: heads * parseInt(m[2], 10), coinFlipNote: `${heads}次正面` };
  }

  // 從自己的手牌選擇N張基本能量卡，附於這隻寶可夢身上。(basic-only approximated to any — documented)
  m = t.match(/^從自己的手牌選擇(\d+)張基本能量卡，附於這隻寶可夢身上。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), attachAnyEnergyFromHandToSelfCount: parseInt(m[1], 10) };

  // 在對手的所有寶可夢身上放置傷害指示物，直到各自身上的傷害指示物的數量變為N倍為止。
  m = t.match(/^在對手的所有寶可夢身上放置傷害指示物，直到各自身上的傷害指示物的數量變為(\d+)倍為止。$/);
  if (m && m[1] === '2') return { baseDamage: parseBaseNumber(damageField), doubleAllOpponentCounters: true };

  // 選擇N個這隻寶可夢身上附加的【T】能量，改附於備戰寶可夢身上。
  m = t.match(/^選擇(\d+)個這隻寶可夢身上附加的【.+?】能量，改附於備戰寶可夢身上。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), moveSelfEnergyToRandomBenchCount: parseInt(m[1], 10) };

  // 擲N次硬幣，若全部為正面，則選擇1隻對手的寶可夢，將其【昏厥】。
  m = t.match(/^擲(\d+)次硬幣，若全部為正面，則選擇1隻對手的寶可夢，將其【昏厥】。$/);
  if (m) {
    const flips = parseInt(m[1], 10);
    const heads = flipCoins(flips);
    if (heads < flips) return { baseDamage: parseBaseNumber(damageField), coinFlipNote: `${heads}次正面` };
    return { baseDamage: parseBaseNumber(damageField), koRandomOpponent: true, coinFlipNote: '全部正面' };
  }

  // 若對手的戰鬥寶可夢處於特殊狀態，則在對手的N隻備戰寶可夢身上放置M個傷害指示物。
  m = t.match(/^若對手的戰鬥寶可夢處於特殊狀態，則在對手的1隻備戰寶可夢身上放置(\d+)個傷害指示物。$/);
  if (m) {
    if (board.defenderStatusConditions.length === 0) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField), placeCountersOnRandomOpponentBench: parseInt(m[1], 10) };
  }

  // 從自己的牌庫選擇任意數量的名稱中有「X」的寶可夢卡，放置於備戰區。並且重洗牌庫。
  m = t.match(/^從自己的牌庫選擇任意數量的名稱中有「(.+?)」的寶可夢卡，放置於備戰區。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchFamilyToBenchCount: { name: m[1], count: 99 } };

  // 對手的1隻寶可夢受到N點傷害。若自己的棄牌區沒有「X」卡M張以上，則這個招式失敗。[...]
  m = t.match(/^對手的1隻寶可夢受到(\d+)點傷害。若自己的棄牌區沒有「(.+?)」卡(\d+)張以上，則這個招式失敗。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) {
    const have = board.ownDiscardCardNames.filter(x => x.includes(m![2])).length;
    if (have < parseInt(m[3], 10)) return { baseDamage: 0 };
    return { baseDamage: parseBaseNumber(damageField), multiTargetOpponentFlatDamage: { count: 1, amount: parseInt(m[1], 10) } };
  }

  // 對手的1隻寶可夢(，)?受到這隻寶可夢身上附加的【T】能量的數量×N點傷害。[...] (comma variant)
  m = t.match(/^對手的1隻寶可夢，受到這隻寶可夢身上附加的【(.+?)】能量的數量×(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[1]];
    if (type) {
      const n = board.attackerEnergyCounts[type] ?? 0;
  ...
  // 從自己的手牌選擇N張「X」卡，將其丟棄。若無法丟棄N張卡，則這個招式失敗。
  m = t.match(/^從自己的手牌選擇(\d+)張「(.+?)」卡，將其丟棄。若無法丟棄\d+張卡，則這個招式失敗。$/);
  if (m) {
    const need = parseInt(m[1], 10);
  ...
  // 若希望，這隻寶可夢也受到N點傷害，將對手的戰鬥寶可夢【狀態】。(auto-taken)
  m = t.match(new RegExp(`^若希望，這隻寶可夢也受到(\\d+)點傷害，將對手的戰鬥寶可夢【(${STATUS_ALT})】。$`));
  if (m) return { baseDamage: parseBaseNumber(damageField), selfDamage: parseInt(m[1], 10), statusToInflict: [STATUS_ZH[m[2]]] };

  // 將自己的1隻備戰寶可夢的HP全部恢復。
  if (/^將自己的1隻備戰寶可夢的HP全部恢復。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), healOneOwnFullHeal: { benchOnly: true } };
  }

  /* ---- Round 4 batch 6 (the last sweep) ---- */

  // 造成自己的場上的，身上放置有傷害指示物的名稱中有「X」的寶可夢的數量×N點傷害。
  m = t.match(/^造成自己的場上的，身上放置有傷害指示物的名稱中有「(.+?)」的寶可夢的數量×(\d+)點傷害。$/);
  if (m) return { baseDamage: board.ownFieldDamagedNames.filter(x => x.includes(m![1])).length * parseInt(m[2], 10) };

  // 若這隻寶可夢的剩餘HP為「N」以下，則增加M點傷害。
  m = t.match(/^若這隻寶可夢的剩餘HP為「(\d+)」以下，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.attackerRemainingHp <= parseInt(m[1], 10) ? parseInt(m[2], 10) : 0) };

  // 擲N次硬幣，從自己的棄牌區選擇最多與正面出現的次數相同數量的(「基本【T】能量」|基本能量)卡，以任意方式附於備戰寶可夢身上。
  m = t.match(/^[‌​]*擲(\d+)次硬幣，從自己的棄牌區選擇最多與正面出現的次數相同數量的(?:「基本【(.+?)】能量」|基本能量)卡，以任意方式附於備戰寶可夢身上。$/);
  if (m) {
    const heads = flipCoins(parseInt(m[1], 10));
    const type = m[2] ? (ENERGY_TYPE_FROM_ZH[m[2]] ?? '') : '';
    return { baseDamage: parseBaseNumber(damageField), discardEnergyToOwnBench: { type, count: heads }, coinFlipNote: `${heads}次正面` };
  }

  // 從自己的牌庫選擇【T】寶可夢卡與競技場卡合計最多N張，在給對手看過後加入手牌。並且重洗牌庫。
  // (the two-kind union is auto-picked — documented)
  m = t.match(/^從自己的牌庫選擇【.+?】寶可夢卡與競技場卡合計最多(\d+)張，在給對手看過後加入手牌。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchAnyCardsToHandCount: parseInt(m[1], 10) };

  // 選擇1隻對手的寶可夢，將那隻寶可夢與附加的卡全部放回對手的牌庫並重洗。(any-position — approximated
  // to the Bench shuffle signal; also feeds the coin-wrapper combinator)
  if (/^選擇1隻對手的寶可夢，將那隻寶可夢與附加的卡，?全部放回對手的牌庫並重洗。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), shuffleOpponentBenchToDeckCount: 1 };
  }

  // 擲1次硬幣若為正面，則從特殊狀態中選擇1種，將對手的戰鬥寶可夢處於那個狀態。
  if (/^擲1次硬幣若為正面，則從特殊狀態中選擇1種，將對手的戰鬥寶可夢處於那個狀態。$/.test(t)) {
    const heads = Math.random() < 0.5;
    if (!heads) return { baseDamage: parseBaseNumber(damageField), coinFlipNote: '反面' };
    const all: StatusCondition[] = ['Asleep', 'Burned', 'Confused', 'Paralyzed', 'Poisoned'];
    return { baseDamage: parseBaseNumber(damageField), statusToInflict: [all[Math.floor(Math.random() * all.length)]], coinFlipNote: '正面' };
  }

  // 若自己剩餘獎賞卡的張數為N張，則增加M點傷害。
  m = t.match(/^若自己剩餘獎賞卡的張數為(\d+)張，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.ownRemainingPrizes === parseInt(m[1], 10) ? parseInt(m[2], 10) : 0) };

  // 若場上有自己的競技場卡，則增加N點傷害。
  m = t.match(/^若場上有自己的競技場卡，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.ownStadiumInPlay ? parseInt(m[1], 10) : 0) };

  // 從雙方的場上寶可夢（這隻寶可夢除外）中選擇1隻剩餘HP最少的寶可夢，將其【昏厥】。
  if (/^從雙方的場上寶可夢（這隻寶可夢除外）中選擇1隻剩餘HP最少的寶可夢，將其【昏厥】。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), koLowestRemainingHpExceptSelf: true };
  }

  // 將對手的戰鬥寶可夢【狀態】。[在備戰區不計算弱點・抵抗力。] (the annotation welds onto the last
  // clause during composition even when it belongs to an earlier damage clause)
  m = t.match(new RegExp(`^將對手的戰鬥寶可夢【(${STATUS_ALT})】。\\s*\\[在備戰區不計算弱點・抵抗力。\\]$`));
  if (m) return { baseDamage: parseBaseNumber(damageField), statusToInflict: [STATUS_ZH[m[1]]] };

  // 將與自己的手牌的張數×N個的相同數量的傷害指示物，放置於對手的戰鬥寶可夢身上。
  m = t.match(/^將與自己的手牌的張數×(\d+)個的相同數量的傷害指示物，放置於對手的戰鬥寶可夢身上。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), placeCountersOnOpponentActive: board.ownHandCount * parseInt(m[1], 10) };

  // 從自己的牌庫選擇最多N張【T】屬性的【基礎】寶可夢卡，放置於備戰區。並且重洗牌庫。(type auto-picked)
  m = t.match(/^從自己的牌庫選擇最多(\d+)張【.+?】屬性的【基礎】寶可夢卡，放置於備戰區。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchBasicPokemonToBenchCount: parseInt(m[1], 10) };

  // 從自己的牌庫任意選擇N張卡加入手牌。並且重洗牌庫。(bare single)
  if (/^從自己的牌庫任意選擇1張卡加入手牌。並且重洗牌庫。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), deckSearchAnyCardToHand: true };
  }

  // 造成對手的棄牌區的物品卡的張數×N點傷害。
  m = t.match(/^造成對手的棄牌區的物品卡的張數×(\d+)點傷害。$/);
  if (m) return { baseDamage: board.opponentDiscardItemCount * parseInt(m[1], 10) };

  // 從自己的牌庫選擇任意數量的「X」，在給對手看過後加入手牌。並且重洗牌庫。
  m = t.match(/^從自己的牌庫選擇任意數量的「(.+?)」，在給對手看過後加入手牌。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchFamilyToHandCount: { name: m[1], count: 99 } };

  // 擲1次硬幣若為正面，則將對手的戰鬥寶可夢【A】與【B】。若為反面，則將對手的戰鬥寶可夢【C】。
  m = t.match(new RegExp(`^擲1次硬幣若為正面，則將對手的戰鬥寶可夢【(${STATUS_ALT})】與【(${STATUS_ALT})】。若為反面，則將對手的戰鬥寶可夢【(${STATUS_ALT})】。$`));
  if (m) {
    const heads = Math.random() < 0.5;
    return {
      baseDamage: parseBaseNumber(damageField),
      statusToInflict: heads ? [STATUS_ZH[m[1]], STATUS_ZH[m[2]]] : [STATUS_ZH[m[3]]],
      coinFlipNote: heads ? '正面' : '反面',
    };
  }

  // 若自己的棄牌區有「X」卡N張以上，則增加M點傷害。
  m = t.match(/^若自己的棄牌區有「(.+?)」卡(\d+)張以上，則增加(\d+)點傷害。$/);
  if (m) {
    const have = board.ownDiscardCardNames.filter(x => x.includes(m![1])).length;
    return { baseDamage: parseBaseNumber(damageField) + (have >= parseInt(m[2], 10) ? parseInt(m[3], 10) : 0) };
  }

  // 從自己的牌庫選擇1張自己的1隻場上寶可夢進化而來的卡，放置於那隻寶可夢身上完成進化。並且重洗牌庫。
  if (/^從自己的牌庫選擇1張自己的1隻場上寶可夢進化而來的卡，放置於那隻寶可夢身上完成進化。並且重洗牌庫。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), evolveOneFieldFromDeck: true };
  }

  // 選擇1個對手的場上寶可夢身上附加的能量，改附於對手的其他寶可夢身上。(approximated to the
  // defender-to-their-bench move signal)
  if (/^選擇1個對手的場上寶可夢身上附加的能量，改附於對手的其他寶可夢身上。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), moveOpponentEnergyToTheirBench: true };
  }

  // 將自己的所有【基礎】寶可夢各恢復「N」HP。
  m = t.match(/^將自己的所有【基礎】寶可夢各恢復「(\d+)」HP。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), healAllOwnBasicsAmount: parseInt(m[1], 10) };

  // 增加這隻寶可夢身上附加的能量的數量×N點傷害。
  m = t.match(/^增加這隻寶可夢身上附加的能量的數量×(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + board.attackerTotalEnergyCount * parseInt(m[1], 10) };

  // 選擇1隻自己的備戰寶可夢，將那隻寶可夢與附加的卡，全部放回手牌。
  if (/^選擇1隻自己的備戰寶可夢，將那隻寶可夢與附加的卡，?全部放回手牌。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), returnOwnBenchToHandCount: 1 };
  }

  // 若對手的戰鬥寶可夢的抵抗力為【T】屬性，則增加N點傷害。
  m = t.match(/^若對手的戰鬥寶可夢的抵抗力為【(.+?)】屬性，則增加(\d+)點傷害。$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[1]];
    const hit = !!type && board.defenderResistanceTypes.includes(type);
    return { baseDamage: parseBaseNumber(damageField) + (hit ? parseInt(m[2], 10) : 0) };
  }

  // 擲與自己的場上【T】寶可夢的數量相同次數的硬幣，造成正面出現的次數×N點傷害。
  m = t.match(/^擲與自己的場上【(.+?)】寶可夢的數量相同次數的硬幣，造成正面出現的次數×(\d+)點傷害。$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[1]];
    const flips = type ? (board.ownFieldTypeCounts[type] ?? 0) : 0;
    const heads = flipCoins(flips);
    return { baseDamage: heads * parseInt(m[2], 10), coinFlipNote: `${heads}次正面` };
  }

  // 對手的1隻寶可夢，受到這隻寶可夢身上附加的能量的數量×N點傷害。[...] (any-energy variant)
  m = t.match(/^對手的1隻寶可夢，受到這隻寶可夢身上附加的能量的數量×(\d+)點傷害。\s*(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), multiTargetOpponentFlatDamage: { count: 1, amount: board.attackerTotalEnergyCount * parseInt(m[1], 10) } };

  // 擲N次硬幣，從自己的棄牌區任意選擇最多與正面出現的次數相同數量的卡，在給對手看過後加入手牌。
  m = t.match(/^擲(\d+)次硬幣，從自己的棄牌區任意選擇最多與正面出現的次數相同數量的卡，在給對手看過後加入手牌。$/);
  if (m) {
    const heads = flipCoins(parseInt(m[1], 10));
    return { baseDamage: parseBaseNumber(damageField), discardPileSearchAnyToHandCount: heads, coinFlipNote: `${heads}次正面` };
  }

  // 從自己的牌庫選擇最多N張【基礎】寶可夢的「X」，放置於備戰區。並且重洗牌庫。
  m = t.match(/^從自己的牌庫選擇最多(\d+)張【基礎】寶可夢的「(.+?)」，放置於備戰區。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchFamilyToBenchCount: { name: m[2].replace(/的寶可夢$/, '的'), count: parseInt(m[1], 10) } };

  // 這隻寶可夢也受到這隻寶可夢身上放置的傷害指示物的數量×N點傷害。
  m = t.match(/^這隻寶可夢也受到這隻寶可夢身上放置的傷害指示物的數量×(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), selfDamage: board.selfDamageCounters * parseInt(m[1], 10) };

  // 將這隻寶可夢與備戰寶可夢互換。然後，從自己的手牌選擇最多N張「X」卡，附於這隻寶可夢身上。
  // (the post-switch attach target is auto-picked — documented)
  m = t.match(/^將這隻寶可夢與備戰寶可夢互換。[‌​]*然後，從自己的手牌選擇最多(\d+)張「(.+?)」卡，附於這隻寶可夢身上。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), selfSwitchToRandomBench: true, attachNamedFromHandCount: { name: m[2], count: parseInt(m[1], 10) } };

  // 從自己的牌庫下方抽出N張卡。
  m = t.match(/^從自己的牌庫下方抽出(\d+)張卡。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), drawFromBottomCount: parseInt(m[1], 10) };

  // 查看自己的牌庫上方N張卡，從其中選擇任意數量的寶可夢卡，在給對手看過後加入手牌。將剩餘卡放回牌庫並重洗。
  m = t.match(/^查看自己的牌庫上方(\d+)張卡，從其中選擇任意數量的寶可夢卡，在給對手看過後加入手牌。將剩餘卡放回牌庫並重洗。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), revealTopPokemonToHandCount: parseInt(m[1], 10) };

  // 若對手的戰鬥寶可夢沒有【撤退】所需的能量，則增加N點傷害。
  m = t.match(/^若對手的戰鬥寶可夢沒有【撤退】所需的能量，則增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + (board.defenderRetreatCost === 0 ? parseInt(m[1], 10) : 0) };

  // 在這個回合，若從手牌使出了「X」，則將對手的牌庫上方N張卡丟棄。
  m = t.match(/^在這個回合，若從手牌使出了「(.+?)」，則將對手的牌庫上方(\d+)張卡丟棄。$/);
  if (m) {
    const played = board.ownSupporterNamesPlayedThisTurn.some(n => n.includes(m![1]));
    if (!played) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField), millOpponentDeckCount: parseInt(m[2], 10) };
  }

  // 從自己的牌庫選擇最多N張基本能量卡，以任意方式附於備戰寶可夢身上。並且重洗牌庫。
  m = t.match(/^從自己的牌庫選擇最多(\d+)張基本能量卡，以任意方式附於備戰寶可夢身上。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchNamedEnergyAttachCount: { name: '基本', count: parseInt(m[1], 10), benchOnly: true } };

  /* ---- Round 4 batch 7 (the very last sweep) ---- */

  // 從自己的牌庫選擇最多N張各不同屬性的基本能量卡，以任意方式附於自己的「X」寶可夢身上。並且重洗牌庫。
  m = t.match(/^從自己的牌庫選擇最多(\d+)張各不同屬性的基本能量卡，以任意方式附於自己的「.+?」寶可夢身上。並且重洗牌庫。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), deckSearchBasicEnergyToOwnPokemonCount: parseInt(m[1], 10) };

  // 造成自己備戰區的所有【T】寶可夢身上放置的傷害指示物的數量×N點傷害。
  m = t.match(/^造成自己備戰區的所有【(.+?)】寶可夢身上放置的傷害指示物的數量×(\d+)點傷害。$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[1]];
    if (type) return { baseDamage: (board.ownBenchTypedDamageCounters[type] ?? 0) * parseInt(m[2], 10) };
  }

  // 對手選擇對手自己的N張手牌，將其丟棄。在這個回合，若這隻寶可夢從「X」進化，則再丟棄M張。
  m = t.match(/^對手選擇對手自己的(\d+)張手牌，將其丟棄。在這個回合，若這隻寶可夢從「.+?」進化，則再丟棄(\d+)張。$/);
  if (m) {
    const n = parseInt(m[1], 10) + (board.attackerEvolvedThisTurn ? parseInt(m[2], 10) : 0);
  ...
  if (/^從自己的牌庫選擇1張【基礎】寶可夢卡，放置於備戰區。並且重洗牌庫。然後，選擇1個這隻寶可夢身上附加的能量，改附於新上場的寶可夢身上。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), benchBasicFromDeckThenMoveEnergy: true };
  }
  ...
  if (m) {
    const played = board.ownSupporterNamesPlayedThisTurn.some(n => n.replace(/^[‌​\s]+/, '').includes(m![2]));
    if (!played) return { baseDamage: 0 };
    return { baseDamage: parseBaseNumber(damageField), gustOpponentBenchThenSplash: { splash: parseInt(m[1], 10) } };
  }

  // 在下個自己的回合，受到這個招式的寶可夢【昏厥】時，多獲得N張獎賞卡。(approximated to the
  // existing any-next-KO bonus counter)
  m = t.match(/^在下個自己的回合，受到這個招式的寶可夢【昏厥】時，多獲得(\d+)張獎賞卡。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), setBonusPrizeNextKo: parseInt(m[1], 10) };

  // 在這個回合，若從手牌使出了「未來/古代」支援者卡，則增加N點傷害。
  m = t.match(/^在這個回合，若從手牌使出了「(未來|古代)」支援者卡，則增加(\d+)點傷害。$/);
  if (m) {
    const hit = m[1] === '未來' ? board.ownPlayedFutureSupporter : board.ownPlayedAncientSupporter;
    return { baseDamage: parseBaseNumber(damageField) + (hit ? parseInt(m[2], 10) : 0) };
  }

  // 將對手的牌庫上方N張卡丟棄。在這個回合，若從手牌使出了「古代/未來」支援者卡，則再丟棄M張。
  m = t.match(/^將對手的牌庫上方(\d+)張卡丟棄。在這個回合，若從手牌使出了「(古代|未來)」支援者卡，則再丟棄(\d+)張。$/);
  if (m) {
    const hit = m[2] === '未來' ? board.ownPlayedFutureSupporter : board.ownPlayedAncientSupporter;
    return { baseDamage: parseBaseNumber(damageField), millOpponentDeckCount: parseInt(m[1], 10) + (hit ? parseInt(m[3], 10) : 0) };
  }

  // 這個招式只可在後攻玩家的最初回合使用。在下個對手的回合，對手無法從手牌使出支援者卡。
  if (/^這個招式只可在後攻玩家的最初回合使用。在下個對手的回合，對手無法從手牌使出支援者卡。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), supporterLockOpponentNextTurn: true };
  }

  // 在下個對手的回合，對手無法從手牌使出寶可夢並完成進化。
  if (/^在下個對手的回合，對手無法從手牌使出寶可夢並完成進化。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), evolutionLockOpponentNextTurn: true };
  }

  // 將場上的對手的競技場卡丟棄。有丟棄的情況下，在下個對手的回合，對手無法從手牌使出競技場卡。
  if (/^將場上的對手的競技場卡丟棄。有丟棄的情況下，在下個對手的回合，對手無法從手牌使出競技場卡。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), discardOpponentStadiumThenLock: true };
  }

  // 將對手的牌庫上方N張卡翻到正面。若希望，選擇1個其中的寶可夢持有的招式，作為這個招式使用。將翻到正面的卡放回牌庫並重洗。
  m = t.match(/^將對手的牌庫上方(\d+)張卡翻到正面。若希望，選擇1個其中的寶可夢持有的招式，作為這個招式使用。將翻到正面的卡放回牌庫並重洗。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), copyFromOpponentDeckTop: { count: parseInt(m[1], 10) } };

  // 在下個自己的回合結束前，受到這個招式的寶可夢弱點改爲【T】屬性。[弱點以「×2」計算傷害。]
  m = t.match(/^在下個自己的回合結束前，受到這個招式的寶可夢弱點改[爲為]【(.+?)】屬性。\s*(?:\[弱點以「×2」計算傷害。\])?$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[1]];
    if (type) {
      // 「在下個自己的回合結束前」 spans the opponent's next turn AND the own turn after it.
      return {
        baseDamage: parseBaseNumber(damageField),
        opponentTimedEffects: [
          { kind: 'weaknessBecomes', vsSubtype: type, turnOffset: 1 },
          { kind: 'weaknessBecomes', vsSubtype: type, turnOffset: 2 },
        ],
      };
    }
  }

  // 若自己的棄牌區有N張以上擁有特性「X」的寶可夢卡，則選擇M隻對手的寶可夢，放置傷害指示物直到各寶可夢身上的傷害指示物數量變為4倍為止。
  m = t.match(/^若自己的棄牌區有(\d+)張以上擁有特性「(.+?)」的寶可夢卡，則選擇(\d+)隻對手的寶可夢，放置傷害指示物直到各寶可夢身上的傷害指示物數量變為4倍為止。$/);
  if (m) {
    if ((board.ownDiscardAbilityCounts[m[2]] ?? 0) < parseInt(m[1], 10)) return { baseDamage: parseBaseNumber(damageField) };
    return { baseDamage: parseBaseNumber(damageField), quadrupleCountersOnOpponents: { count: parseInt(m[3], 10) } };
  }

  // 在下個對手的回合，受到這個招式的寶可夢無法撤退。在下個自己的回合，受到這個招式的寶可夢受到招式的傷害「+N」點。
  m = t.match(/^在下個對手的回合，受到這個招式的寶可夢無法撤退。在下個自己的回合，受到這個招式的寶可夢受到招式的傷害「\+(\d+)」點。$/);
  if (m) {
    return {
      baseDamage: parseBaseNumber(damageField),
      opponentTimedEffects: [
        { kind: 'cantRetreat', turnOffset: 1 },
        // Negative damageReduction = the defender takes that much MORE.
        { kind: 'damageReduction', amount: -parseInt(m[1], 10), turnOffset: 2 },
      ],
    };
  }
  ...
  // 將自己的牌庫下方N張卡翻到正面，造成其中持有招式「X」的寶可夢卡的張數×M點傷害。將翻到正面的寶可夢卡放回牌庫並重洗。將剩餘卡丟棄。
  m = t.match(/^將自己的牌庫下方(\d+)張卡翻到正面，造成其中持有招式「(.+?)」的寶可夢卡的張數×(\d+)點傷害。將翻到正面的寶可夢卡放回牌庫並重洗。將剩餘卡丟棄。$/);
  if (m) {
    return { baseDamage: 0, revealBottomAttackHolderScaledDamage: { count: parseInt(m[1], 10), attackName: m[2], amount: parseInt(m[3], 10) } };
  }

  // 將這隻寶可夢身上附加的任意數量的基本能量卡丟棄，造成其張數×N點傷害。
  m = t.match(/^將這隻寶可夢身上附加的任意數量的基本能量卡丟棄，造成其張數×(\d+)點傷害。$/);
  if (m) return { baseDamage: 0, selfEnergyDiscardScaledDamage: { amount: parseInt(m[1], 10) } };

  // 若希望，將N個這隻寶可夢身上附加的【T】能量丟棄，增加M點傷害。
  m = t.match(/^若希望，將(\d+)個這隻寶可夢身上附加的【(.+?)】能量丟棄，增加(\d+)點傷害。$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[2]];
    if (type) return { baseDamage: parseBaseNumber(damageField), selfEnergyPaymentRider: { count: parseInt(m[1], 10), type, amount: parseInt(m[3], 10) } };
  }

  // 若希望，將這隻寶可夢身上附加的能量卡全部丟棄，增加N點傷害。
  m = t.match(/^若希望，將這隻寶可夢身上附加的能量卡全部丟棄，增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), selfEnergyPaymentRider: { all: true, amount: parseInt(m[1], 10) } };

  // 若希望，將這隻寶可夢身上附加的能量卡全部放回牌庫並重洗，增加N點傷害。
  m = t.match(/^若希望，將這隻寶可夢身上附加的能量卡全部放回牌庫並重洗，增加(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), selfEnergyPaymentRider: { all: true, toDeck: true, amount: parseInt(m[1], 10) } };

  // (若希望，)將N個這隻寶可夢身上附加的(【T】)能量丟棄，將對手的戰鬥寶可夢【X】。
  m = t.match(/^(?:若希望，)?將(\d+)個這隻寶可夢身上附加的(?:【(.+?)】)?能量丟棄，將對手的戰鬥寶可夢【(.+?)】。$/);
  if (m) {
    const status = STATUS_ZH[m[3]];
    const type = m[2] ? ENERGY_TYPE_FROM_ZH[m[2]] : undefined;
    if (status && (!m[2] || type)) {
      return { baseDamage: parseBaseNumber(damageField), selfEnergyPaymentRider: { count: parseInt(m[1], 10), type, status } };
    }
  }

  // 對於對手的所有寶可夢，各自擲1次硬幣，所有出現正面的寶可夢，各受到N點傷害。
  m = t.match(/^對於對手的所有寶可夢，各自擲1次硬幣，所有出現正面的寶可夢，各受到(\d+)點傷害。(?:\[在備戰區不計算弱點・抵抗力。\])?$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), coinPerOpponentPokemonDamage: { amount: parseInt(m[1], 10) } };

  // 選擇任意數量的對手的備戰寶可夢身上放置的傷害指示物，改放於對手的戰鬥寶可夢身上。
  if (/^選擇任意數量的對手的備戰寶可夢身上放置的傷害指示物，改放於對手的戰鬥寶可夢身上。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), moveOpponentBenchCountersToActive: true };
  }

  // 將這隻寶可夢身上附加的能量卡全部丟棄，在對手的1隻寶可夢身上放置N個傷害指示物。
  m = t.match(/^將這隻寶可夢身上附加的能量卡全部丟棄，在對手的1隻寶可夢身上放置(\d+)個傷害指示物。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField), discardAllSelfEnergyForCounters: { counters: parseInt(m[1], 10) } };

  // 選擇N個這隻寶可夢身上附加的【T】能量，放回手牌。
  m = t.match(/^選擇(\d+)個這隻寶可夢身上附加的【(.+?)】能量，放回手牌。$/);
  if (m) {
    const type = ENERGY_TYPE_FROM_ZH[m[2]];
    if (type) return { baseDamage: parseBaseNumber(damageField), returnSelfTypedEnergyToHand: { type, count: parseInt(m[1], 10) } };
  }

  // 增加上個對手的回合對手獲得的獎賞卡的張數×N點傷害。
  m = t.match(/^增加上個對手的回合對手獲得的獎賞卡的張數×(\d+)點傷害。$/);
  if (m) return { baseDamage: parseBaseNumber(damageField) + board.opponentPrizesTakenLastTurn * parseInt(m[1], 10) };

  // 選擇1個這隻寶可夢身上附加的基本能量，改附於備戰寶可夢身上。
  if (/^選擇1個這隻寶可夢身上附加的基本能量，改附於備戰寶可夢身上。$/.test(t)) {
    return { baseDamage: parseBaseNumber(damageField), moveSelfEnergyToRandomBench: true };
  }
...
/** CLAUSE-LEADING references to a previous clause — a later clause starting with one of these
 * can't be resolved independently, so the text must not be composed. Mid-clause occurrences
 * (e.g. 「擲1次硬幣若為正面，則…」 as one clause, or 「…那隻寶可夢…」 referring within its own
 * clause) are legitimate; a genuinely dangling reference simply matches no template and keeps
 * the text uncovered anyway. */
const CROSS_CLAUSE_REF = /^(?:這個情況下|若為正面，|若為反面，)/;
