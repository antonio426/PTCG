...
- 卡片 ID 格式為 `{SetCode}-{Number}`（例如 `SV1V-001`）。**舊 ID 問題已完全解決**：預組已重指到 Standard 印刷（`819a278`）；使用者存在 `localStorage` 的舊 ID 牌組（輪替出去的印刷如 `S5R-027`、舊爬蟲 `scr-*`）會在 app 載入時自動遷移一次——`printRemap.ts`（與 `audit-preset-decks-standard.ts` 共用）+ `POST /api/cards/remap` + `deckStore` 的遷移（原始資料備份到 `ptcg-decks-pre-migration`、`ptcg-decks-migrated-v1` 戳記防重跑；store 在 `App.tsx` 以副作用 import 載入，因為 lazy-loaded 路由從首頁進站時永遠不會觸發它）。`deckStore.validateDeck` 仍容忍未知 ID 作為最後防線。`AGENTS.md` 僅作歷史記錄——其架構段落已過時（檔首有聲明）。
